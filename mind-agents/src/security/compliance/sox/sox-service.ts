...
export class SOXServiceImpl extends EventEmitter implements SOXService {
  ...
  tagFinancialData(data: any): FinancialDataTag {
    ...
    const requiresApproval = materialityLevel !== 'immaterial' || 
      detectedTypes.some(type => ['revenue', 'expense', 'asset', 'liability'].includes(type));
  ...
  async validateFinancialTransaction(transaction: any): Promise<ValidationResult> {
    try {
      ...
      if (tag.materialityLevel === 'highly_material' && !transaction.approvals?.length) {
        errors.push('Highly material transactions require management approval');
      }

      // Check segregation of duties
      if (transaction.createdBy === transaction.approvedBy) {
        errors.push('Segregation of duties violation: same person cannot create and approve');
      }

      // Validate account codes
      if (transaction.accountCode && !/^\d{4,6}$/.test(transaction.accountCode)) {
      ...
      if (transaction.entryType === 'manual') {
        ...
        if (hour < this.config.changeWindowHours.start || hour > this.config.changeWindowHours.end) {
      ...
      await this.logChange('financial_transaction', {
        before: {},
        after: transaction,
      ...
      runtimeLogger.error('Failed to validate financial transaction', { error });
      return {
        valid: false,
   ...
   */
  async requestChange(change: Omit<ChangeRequest, 'id' | 'requestedAt' | 'status'>): Promise<ChangeRequest> {
    try {
      ...
      await this.logChange('change_request', {
        before: {},
        after: changeRequest,
      ...
      runtimeLogger.info('Change request created', { changeId: changeRequest.id, type: change.type });
   ...
   */
  async approveChange(changeId: string, approval: Omit<ChangeApproval, 'approvedAt'>): Promise<void> {
    try {
      ...
      // Check segregation of duties
      const canApprove = await this.validateSegregationOfDuties(approval.approverId, 'approve_change');
      if (!canApprove) {
        throw new Error('Segregation of duties violation: user cannot approve changes');
      }
      ...
      await this.logChange('change_approval', {
        before: { status: 'pending', approvals: changeRequest.approvals.length - 1 },
        after: { status: changeRequest.status, approvals: changeRequest.approvals.length },
   ...
   */
  async implementChange(changeId: string, implementation: ChangeImplementation): Promise<void> {
    try {
      const changeRequest = await this.getChangeRequest(changeId);
      if (!changeRequest) {
        throw new Error(`Change request ${changeId} not found`);
      ...
      if (implementation.implementedBy === changeRequest.requestedBy) {
        throw new Error('Segregation of duties violation: requestor cannot implement their own change');
      }

      const approverIds = changeRequest.approvals.map(a => a.approverId);
      if (approverIds.includes(implementation.implementedBy)) {
        throw new Error('Segregation of duties violation: approver cannot implement the change');
      }

      // Validate test results
      const failedTests = implementation.testResults.filter(test => test.result === 'fail');
      if (failedTests.length > 0) {
        throw new Error(`Implementation has failed tests: ${failedTests.map(t => t.name).join(', ')}`);
      }

      if (!implementation.rollbackTested) {
        throw new Error('Rollback procedure must be tested before implementation');
      }
      ...
      await this.logChange('change_implementation', {
   ...
   */
  async validateSegregationOfDuties(userId: string, action: string): Promise<boolean> {
    try {
      ...
      switch (action) {
        ...
        case 'implement_change':
          return !userRoles.includes('approver') && !userRoles.includes('requestor');
      ...
      runtimeLogger.error('Failed to validate segregation of duties', { userId, action, error });
      return false;
    }
  }
   ...
   */
  async logChange(entity: string, changes: ChangeSet, userId: string): Promise<void> {
    ...
    const relevantControls = Array.from(this.controls.values())
      .filter(control => this.isControlRelevant(control, entity));
  ...
  async testControl(controlId: string): Promise<ControlTestResult> {
    try {
      ...
      const exceptionsFound = await this.performControlTest(control, sampleSize);
      
      const effectiveness: 'effective' | 'partially_effective' | 'ineffective' = 
        exceptionsFound === 0 ? 'effective' :
        exceptionsFound <= sampleSize * 0.05 ? 'partially_effective' : // 5% tolerance
        'ineffective';
      ...
      runtimeLogger.info('Control tested', { controlId, effectiveness, exceptionsFound });
  ...
  async getControlEffectiveness(): Promise<ControlEffectivenessReport> {
    try {
      ...
      runtimeLogger.error('Failed to get control effectiveness report', { error });
      throw error;
    }
  }
  ...
  async generateSOXReport(period: ReportPeriod): Promise<SOXComplianceReport> {
    try {
      ...
      return {
        ...
        significantDeficiencies,
        remediationPlans,
        auditOpinion: this.determineAuditOpinion(materialWeaknesses, significantDeficiencies),
      };
    } catch (error) {
  ...
  private async performControlTest(control: SOXControl, sampleSize: number): Promise<number> {
  ...
  private generateFindings(control: SOXControl, exceptionsFound: number): string[] {
    const findings: string[] = [];

    if (exceptionsFound === 0) {
      findings.push(`Control ${control.id} operated effectively with no exceptions found`);
    } else {
      findings.push(`Control ${control.id} had ${exceptionsFound} exceptions`);
      
      if (exceptionsFound > 5) {
        findings.push('High number of exceptions indicates potential control weakness');
      }
      
      switch (control.category) {
        case 'access':
          findings.push('Access control exceptions require immediate attention');
          break;
        case 'segregation_of_duties':
  ...
  private generateRecommendations(control: SOXControl, effectiveness: string): string[] {
  ...
  private generateKeyFindings(controls: SOXControl[]): string[] {
    ...
    if (ineffectiveControls > 0) {
      findings.push(`${ineffectiveControls} controls are ineffective and require remediation`);
    }
    ...
    const effectiveAccessControls = accessControls.filter(c => c.effectiveness === 'effective').length;
    
    if (effectiveAccessControls / accessControls.length < 0.8) {
      findings.push('Access controls effectiveness is below acceptable threshold');
    }
  ...
  private generateExecutiveSummary(
    ...
    const effectivenessRate = (controlEffectiveness.effectiveControls / controlEffectiveness.totalControls) * 100;
    ...
    if (materialWeaknesses.length === 0 && significantDeficiencies.length === 0) {
      summary += 'No material weaknesses or significant deficiencies identified.';
    }
  ...
  private async storeChangeRequest(changeRequest: ChangeRequest): Promise<void> {
    await this.memoryProvider.storeMemory({
      id: changeRequest.id,
      agentId: 'system',
  ...
  private async updateChangeRequest(changeRequest: ChangeRequest): Promise<void> {
    await this.storeChangeRequest(changeRequest);
  }
