...
export class SupabaseChatRepository implements ChatRepository {
  ...
  private async initializeSchema(): Promise<void> {
    try {
      ...
      if (error && error.message.includes('relation "conversations" does not exist')) {
        console.log('📋 Chat tables not found. Please run the Supabase chat schema SQL manually.')
        console.log('Schema location: src/modules/memory/providers/supabase/chat-schema.sql')
      }
    } catch (error) {
  ...
  async createMessage(message: Omit<Message, 'id' | 'timestamp'>): Promise<Message> {
    ...
    console.log(`📝 Created message ${result.id} in conversation ${message.conversationId}`)
    return result
  }
  ...
  async searchMessages(conversationId: string, searchText: string, limit = 50): Promise<Message[]> {
    ...
    try {
      const { data: semanticResults, error: semanticError } = await this.client
        .rpc('search_messages_by_similarity', {
  ...
  async getConversationStats(conversationId: string): Promise<ConversationStats> {
    ...
    return {
      ...
      firstMessageAt: data.first_message_at ? new Date(data.first_message_at) : undefined,
  ...
  private supabaseToConversationWithLastMessage(row: any): ConversationWithLastMessage {
    return {
 ...
 */
export function createSupabaseChatRepository(config: SupabaseChatConfig): SupabaseChatRepository {
