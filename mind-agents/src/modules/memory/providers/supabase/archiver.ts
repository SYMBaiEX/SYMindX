...
export class MemoryArchiver {
   ...
   */
  private async compressMemories(memories: MemoryRecord[], strategy: ArchivalStrategy): Promise<MemoryRecord[]> {
   ...
   */
  private async summarizeMemories(memories: MemoryRecord[], strategy: ArchivalStrategy): Promise<MemoryRecord[]> {
   ...
   */
  private async hierarchicalArchive(memories: MemoryRecord[], strategy: ArchivalStrategy): Promise<MemoryRecord[]> {
    // Group memories by importance and age for hierarchical storage
    const highImportance = memories.filter(m => (m.importance || 0) > 0.8)
    const mediumImportance = memories.filter(m => (m.importance || 0) > 0.5 && (m.importance || 0) <= 0.8)
    ...
    const compressedMedium = await this.compressMemories(mediumImportance, strategy)
