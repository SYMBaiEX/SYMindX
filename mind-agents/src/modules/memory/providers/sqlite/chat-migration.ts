...
export class ChatMigrationManager {
  ...
  private getAppliedVersions(): string[] {
    const stmt = this.db.prepare('SELECT version FROM schema_migrations ORDER BY applied_at')
  ...
  async validate(): Promise<{ valid: boolean; errors: string[] }> {
    ...
    try {
      ...
      const isOk = integrityCheck === 'ok' || 
                   ...
                   (Array.isArray(integrityCheck) && integrityCheck.length === 1 && 
                    typeof integrityCheck[0] === 'object' && (integrityCheck[0] as any).integrity_check === 'ok') ||
                   (typeof integrityCheck === 'object' && integrityCheck !== null && (integrityCheck as any).integrity_check === 'ok')
      if (!isOk) {
        errors.push(`Database integrity check failed: ${JSON.stringify(integrityCheck)}`)
      }
      ...
      if (existingTables.length > 1) { // More than just schema_migrations table
  ...
  addMigration(migration: Migration): void {
    ...
    if (!migration.version || !migration.description || !migration.up) {
      throw new Error('Migration must have version, description, and up script')
    }
 ...
 */
export function createChatMigrationManager(dbPath: string): ChatMigrationManager {
