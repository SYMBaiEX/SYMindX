    ...
    try {
      ...
      try {
        ...
        if (parseInt(result.rows[0].count) === 0) {
          console.log('📋 Chat tables not found. Please run the Neon chat schema SQL.')
          console.log('Schema location: src/modules/memory/providers/neon/chat-schema.sql')
        }
      } finally {
        client.release()
      }
 ...
 */
export function createNeonChatRepository(config: NeonChatConfig): NeonChatRepository {
