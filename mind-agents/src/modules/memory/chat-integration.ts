...
export interface ChatFactoryConfig {
  ...
  config: ChatSystemConfig | SupabaseChatConfig | NeonChatConfig | PostgresChatConfig
}

/**
 * Main factory function for creating chat repositories
 */
export function createChatRepository(options: ChatFactoryConfig): ChatRepository {
...
export async function getChatSystemStatus(repository: ChatRepository): Promise<{
  ...
  try {
    ...
    const status = details.canCreateConversation && details.canCreateMessage && details.canQueryMessages
      ? 'healthy'
      : 'degraded'
