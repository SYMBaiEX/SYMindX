...
export abstract class BaseMemoryProvider implements MemoryProvider {
   ...
   */
  async storeTier(agentId: string, memory: MemoryRecord, tier: MemoryTierType): Promise<void> {
   ...
   */
  protected async addToWorkingMemory(agentId: string, memory: MemoryRecord): Promise<void> {
    ...
    if (items.length >= capacity) {
      ...
      if (leastAttended.attention > 0.5) {
        await this.consolidateMemory(agentId, leastAttended.id, MemoryTierType.WORKING, MemoryTierType.EPISODIC)
      }
   ...
   * @param limit Maximum number of memories to return
   */
  abstract retrieveTier(agentId: string, tier: MemoryTierType, limit?: number): Promise<MemoryRecord[]>
   ...
   */
  protected updateWorkingMemoryAttention(agentId: string, memoryId: string): void {
    const items = this.workingMemory.get(agentId) || []
    const tier = this.tiers.get(MemoryTierType.WORKING)
   ...
   * @param limit Maximum number of memories to return
   */
  async retrieveSemantic(agentId: string, concept: string, limit: number = 10): Promise<MemoryRecord[]> {
    const memories = await this.retrieveTier(agentId, MemoryTierType.SEMANTIC, limit * 2)
    ...
    return memories
      .filter(memory => {
        ...
        return content.includes(conceptLower) || 
  ...
  async retrieveEpisodic(
    ...
    const memories = await this.retrieveTier(agentId, MemoryTierType.EPISODIC, limit * 2)
   ...
   */
  abstract shareMemories(agentId: string, memoryIds: string[], poolId: string): Promise<void>
   ...
   */
  protected cosineSimilarity(embedding1: number[], embedding2: number[]): number {
