...
export class MemoryManagementEngine {
  ...
  applyDecay(
    ...
    const decayPolicy = this.policies.find(p => p.type === 'decay' && p.enabled)
    ...
    const decayFunction = this.getDecayFunction(config.decayFunction || 'exponential')
    ...
    for (const memory of memories) {
      ...
      if (newImportance > importanceThreshold) {
        ...
        if (lastAccess) {
          const daysSinceAccess = (now - lastAccess.getTime()) / (1000 * 60 * 60 * 24)
  ...
  prioritizeMemories(
    ...
    const now = Date.now()
    const relationshipCounts = this.calculateRelationshipCounts(relationships || [])

    for (const memory of memories) {
      // Check cache first
      const cached = this.priorityCache.get(memory.id)
      if (cached && (now - cached.calculated.getTime()) < 60 * 60 * 1000) { // 1 hour cache
        priorities.push(cached)
        continue
      }

      // Calculate importance score
      const importanceScore = memory.importance * (priorityFactors.importance || 0)

      // Calculate recency score
      const age = (now - memory.timestamp.getTime()) / (1000 * 60 * 60 * 24)
      const recencyScore = Math.max(0, 1 - age / 30) * (priorityFactors.recency || 0)
      ...
      const relationshipScore = Math.min(1, relationshipCount / 5) * (priorityFactors.relationshipCount || 0)

      // Calculate total priority
      const totalPriority = importanceScore + recencyScore + accessScore + emotionalScore + relationshipScore
  ...
  async summarizeCluster(
    ...
    const originalSize = cluster.memories.reduce((sum, m) => sum + m.content.length, 0)
  ...
  cleanupByPriority(
    ...
    const priorityMap = new Map(priorities.map(p => [p.memoryId, p.priority]))
   ...
   */
  private calculateRelationshipCounts(relationships: MemoryRelationship[]): Map<string, number> {
   ...
   */
  private clusterByTime(memories: MemoryRecord[], config: MemoryPolicyConfig): MemoryCluster[] {
    ...
    for (const memory of sortedMemories) {
      ...
      if (lastTimestamp === null || timestamp - lastTimestamp <= timeThreshold) {
   ...
   */
  private clusterByEmbedding(memories: MemoryRecord[], config: MemoryPolicyConfig): MemoryCluster[] {
    const clusters: MemoryCluster[] = []
   ...
   */
  private clusterByConcepts(memories: MemoryRecord[], config: MemoryPolicyConfig): MemoryCluster[] {
    const clusters: MemoryCluster[] = []
   ...
   */
  private calculateContentSimilarity(memory1: MemoryRecord, memory2: MemoryRecord): number {
  ...
  private kMeansCluster(memories: MemoryRecord[], k: number): MemoryRecord[][] {
    const embeddings = memories.map(m => m.embedding!).filter(e => e !== undefined)
    ...
    while (!converged && iterations < maxIterations) {
      const clusters: MemoryRecord[][] = Array(k).fill(null).map(() => [])
    ...
    // Final assignment
    const finalClusters: MemoryRecord[][] = Array(k).fill(null).map(() => [])
    for (let i = 0; i < memories.length; i++) {
      const embedding = memories[i].embedding!
      let nearestCentroid = 0
      let minDistance = Infinity
  ...
  getStats(): {
    ...
    const totalAccesses = Array.from(this.accessTracker.values())
      .reduce((sum, accesses) => sum + accesses.reduce((s, a) => s + a.frequency, 0), 0)
