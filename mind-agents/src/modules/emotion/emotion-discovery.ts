...
export class EmotionDiscovery {
  ...
  private async discoverBuiltInEmotions(): Promise<DiscoveredEmotion[]> {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && !['base-emotion.ts', 'composite-emotion.ts'].includes(entry.name)) {
  ...
  private async discoverLocalEmotions(): Promise<DiscoveredEmotion[]> {
    const emotions: DiscoveredEmotion[] = []
    const localEmotionsDir = path.join(path.dirname(this.emotionsDir), '..', '..', 'emotions')
        ...
        if (entry.isDirectory()) {
          ...
          if (fs.existsSync(packageJsonPath)) {
            try {
              ...
              runtimeLogger.warn(`⚠️ Failed to parse package.json for local emotion ${entry.name}:`, error)
            }
          }
        }
      }
  ...
  async autoRegisterEmotions(registry: ModuleRegistry): Promise<void> {
    ...
    for (const emotion of emotions) {
      try {
        ...
        const shouldAutoRegister = emotion.packageInfo.symindx?.emotion?.autoRegister !== false
   ...
   */
  async registerEmotion(registry: ModuleRegistry, emotion: DiscoveredEmotion): Promise<void> {
    try {
      ...
      if (emotion.type === 'built-in') {
        ...
        modulePath = path.join(emotion.path, emotion.packageInfo.main || 'index')
      }
      ...
      runtimeLogger.error(`❌ Failed to register emotion ${emotion.name}:`, error)
      throw error
    }
  }
  ...
  private inferEmotionCategory(emotionName: string): string {
    const basicEmotions = ['happy', 'sad', 'angry', 'fearful', 'surprised', 'disgusted']
    const complexEmotions = ['nostalgic', 'melancholic', 'euphoric', 'serene', 'anxious']
    const socialEmotions = ['empathetic', 'envious', 'grateful', 'guilty', 'proud']
    const cognitiveEmotions = ['curious', 'confused', 'confident', 'frustrated', 'satisfied']
  ...
  private getDefaultTriggers(emotionName: string): string[] {
    const triggerMap: Record<string, string[]> = {
      happy: ['success', 'achievement', 'positive_feedback', 'praise', 'victory'],
      ...
      confident: ['success', 'mastery', 'recognition', 'accomplishment', 'validation'],
      curious: ['novelty', 'mystery', 'learning', 'exploration', 'question'],
      proud: ['achievement', 'recognition', 'success', 'accomplishment', 'validation'],
      empathetic: ['others_emotion', 'suffering', 'connection', 'understanding', 'caring'],
      confused: ['complexity', 'ambiguity', 'contradiction', 'uncertainty', 'overload'],
