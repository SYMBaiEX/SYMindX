...
export class TheoryOfMind {
   ...
   */
  predict(agentId: string, situation: string): {
    action: string
    confidence: number
    reasoning: string
  } {
    ...
    const emotionalInfluence = this.calculateEmotionalInfluence(model, situation)
  ...
  empathize(agentId: string): {
    ...
    // Apply empathy level
    const empathizedIntensity = model.emotionalState.intensity * this.config.empathyLevel!
  ...
  private analyzeCommunicationStyle(model: MentalModel, message: string): void {
    ...
    const informalIndicators = ['hey', 'yeah', 'nah', 'gonna', 'wanna', 'lol', 'omg']
    
    const formalCount = formalIndicators.filter(i => message.toLowerCase().includes(i)).length
    const informalCount = informalIndicators.filter(i => message.toLowerCase().includes(i)).length
    
    // Update formality with momentum
    const formalityDelta = (formalCount - informalCount) * 0.1
    model.communicationStyle.formality = Math.max(0, Math.min(1, 
      model.communicationStyle.formality + formalityDelta
    ))
    
    // Verbosity analysis
    const wordCount = message.split(/\s+/).length
    const verbosityDelta = (wordCount > 20 ? 0.05 : -0.05)
    model.communicationStyle.verbosity = Math.max(0, Math.min(1,
      model.communicationStyle.verbosity + verbosityDelta
    ))
    
    // Directness analysis
    const directIndicators = ['want', 'need', 'must', 'will', 'do', 'don\'t']
    const directCount = directIndicators.filter(i => message.toLowerCase().includes(i)).length
    const directnessDelta = directCount * 0.05
    model.communicationStyle.directness = Math.max(0, Math.min(1,
      model.communicationStyle.directness + directnessDelta
    ))
  }
   ...
   * Update emotional model
   */
  private updateEmotionalModel(model: MentalModel, emotion: EmotionState): void {
    ...
    model.emotionalState.history = model.emotionalState.history
      .filter(h => h.timestamp.getTime() > oneHourAgo)
  }
  
  /**
   * Update intentions based on actions
   */
  private updateIntentions(model: MentalModel, action: string, context: any): void {
    ...
    model.intentions = model.intentions
      .map(i => i.action === action ? i : { ...i, likelihood: i.likelihood * 0.95 })
  ...
  private updateRelationship(model: MentalModel, observation: any): void {
    ...
    if (observation.message?.toLowerCase().includes('thank') ||
        observation.message?.toLowerCase().includes('appreciate')) {
      model.relationship.rapport = Math.min(1, model.relationship.rapport + 0.05)
    }
    
    // Repeated interactions increase familiarity
    model.relationship.familiarity = Math.min(1, model.relationship.familiarity + 0.01)
    
    // Trust builds slowly over positive interactions
    if (observation.emotion?.current === 'happy' || observation.emotion?.current === 'grateful') {
   ...
   */
  private calculateEmotionalInfluence(model: MentalModel, situation: string): number {
...
export function createTheoryOfMindCognition(config: TheoryOfMindConfig = {}): TheoryOfMind {
  return new TheoryOfMind(config)
}