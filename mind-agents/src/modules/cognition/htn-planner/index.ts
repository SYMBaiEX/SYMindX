...
export class HTNPlannerCognition implements CognitionModule {
  ...
  async think(agent: Agent, context: ThoughtContext): Promise<ThoughtResult> {
    ...
    for (const goal of goals) {
      ...
      plans.push(plan)
      thoughts.push(`Decomposed goal "${goal}" into ${plan.steps.length} steps`)
    }
  ...
  private async decomposeGoal(goal: string, network: TaskNetwork): Promise<Task[]> {
    // Check cache first
    const cacheKey = `${goal}_${network.tasks.length}`
    if (this.config.cacheDecompositions && this.decompositionCache.has(cacheKey)) {
  ...
  private assessEmotionalResponse(context: ThoughtContext, plans: Plan[]): EmotionState {
  ...
  private async rankOptions(options: Decision[], agent: Agent): Promise<Decision[]> {
  ...
  private scoreOption(option: Decision, agent: Agent): number {
    ...
    if (option.reasoning?.includes('plan') || option.reasoning?.includes('strategy')) {
      score += 0.2
    }
    
    // Boost score for goal-aligned options
    if (option.reasoning?.includes('goal') || option.reasoning?.includes('objective')) {
      score += 0.1
    }
