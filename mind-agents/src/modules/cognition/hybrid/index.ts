...
export class HybridCognition implements CognitionModule {
  ...
  private performanceHistory: Array<{ approach: 'reactive' | 'planning'; success: boolean }> = []
  ...
  async think(agent: Agent, context: ThoughtContext): Promise<ThoughtResult> {
    ...
    // Add hybrid metadata
    result.thoughts.unshift(`Using ${approach} approach (${Math.round(contextAnalysis.complexity * 100)}% complexity)`)
  ...
  private async reactiveThinking(agent: Agent, context: ThoughtContext, analysis: any): Promise<ThoughtResult> {
  ...
  private async planningThinking(agent: Agent, context: ThoughtContext, analysis: any): Promise<ThoughtResult> {
    const startTime = Date.now()
  ...
  private assessComplexity(context: ThoughtContext | any): number {
    ...
    if (context.goal) {
      ...
      const technicalTerms = ['algorithm', 'framework', 'implementation', 'architecture']
  ...
  private assessUrgency(context: ThoughtContext | Decision[]): number {
    if (Array.isArray(context)) {
      ...
      return context.some(option => 
        option.reasoning?.includes('urgent') || option.reasoning?.includes('immediate')
      ) ? 0.9 : 0.3
    }
    ...
    if (context.goal) {
      const urgentWords = ['urgent', 'immediate', 'asap', 'emergency', 'critical', 'now']
  ...
  private detectEmotionalContent(context: ThoughtContext): number {
    ...
    const emotionalWords = ['feel', 'emotion', 'happy', 'sad', 'angry', 'excited', 'worried']
  ...
  private detectGoalOrientation(context: ThoughtContext): number {
    ...
    const goalWords = ['goal', 'plan', 'achieve', 'accomplish', 'objective', 'target']
  ...
  private detectPlanningNeeds(context: ThoughtContext): number {
    ...
    const planningWords = ['plan', 'strategy', 'approach', 'method', 'steps', 'process']
  ...
  private generatePlannedActions(context: ThoughtContext, analysis: any): string[] {
  ...
  private quickEmotionalAssessment(context: ThoughtContext): EmotionState | undefined {
  ...
  private deliberateEmotionalAssessment(context: ThoughtContext, analysis: any): EmotionState | undefined {
  ...
  private recordPerformance(approach: 'reactive' | 'planning', success: boolean): void {
  ...
  private adaptWeights(): void {
    ...
    const reactiveSuccess = recentHistory.filter(h => h.approach === 'reactive' && h.success).length
    const planningSuccess = recentHistory.filter(h => h.approach === 'planning' && h.success).length
    const reactiveTotal = recentHistory.filter(h => h.approach === 'reactive').length
    const planningTotal = recentHistory.filter(h => h.approach === 'planning').length
    ...
    if (reactiveTotal > 0 && planningTotal > 0) {
      ...
      if (reactiveRate > planningRate) {
        this.config.reactiveWeight = Math.min(0.8, this.config.reactiveWeight! + adaptationRate)
        this.config.planningWeight = Math.max(0.2, this.config.planningWeight! - adaptationRate)
      } else {
        this.config.planningWeight = Math.min(0.8, this.config.planningWeight! + adaptationRate)
        this.config.reactiveWeight = Math.max(0.2, this.config.reactiveWeight! - adaptationRate)
      }
    }
  }
}
