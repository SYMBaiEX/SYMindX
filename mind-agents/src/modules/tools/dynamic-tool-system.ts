...
export class DynamicToolSystem implements ToolSystem {
  ...
  async initialize(agent: Agent): Promise<void> {
    ...
    try {
      ...
      runtimeLogger.info(`🔧 Dynamic tool system initialized with ${this.tools.size} tools`)
    } catch (error) {
  ...
  async executeTool(name: string, args: ToolParameters): Promise<ToolExecutionResult> {
  ...
  getStatus() {
    ...
    return {
      ...
      averageExecutionTime: this.executionStats.executionCount > 0 
  ...
  getToolsByServer(serverName: string): Map<string, ToolDefinition> {
    ...
    for (const [toolName, tool] of this.tools) {
      if (tool.metadata?.server === serverName || toolName.startsWith(`${serverName}:`)) {
   ...
   */
  private getServerStats(): Record<string, { toolCount: number; tools: string[] }> {
  ...
  private async discoverMCPTools(): Promise<void> {
    ...
    const mcpExtension = Array.from(this.agent.extensions || [])
      .find(ext => ext.id === 'mcp-client')
    ...
    try {
      ...
      if (typeof (mcpExtension as any).getAvailableTools !== 'function') {
        runtimeLogger.debug('🔧 MCP extension does not provide getAvailableTools method')
        return
      }
      ...
      for (const [toolKey, mcpTool] of mcpTools) {
        ...
        const toolDefinition: ToolDefinition = {
          ...
          execute: async (args: ToolParameters): Promise<ToolResult> => {
            const result = await (mcpExtension as any).executeTool(toolKey, args)
            if (result.content && Array.isArray(result.content)) {
              return result.content.map((c: any) => c.text || c.data || '').join('\n')
            }
        ...
        if (this.config.logging.enabled && this.config.logging.level === 'debug') {
          runtimeLogger.debug(`🔧 Registered MCP tool: ${namespacedToolName} from server ${serverName}`)
        }
      }
      
      runtimeLogger.info(`🔧 Auto-registered ${totalToolsRegistered} tools from ${this.getUniqueServerCount(mcpTools)} MCP servers`)
    } catch (error) {
  ...
  private getUniqueServerCount(mcpTools: Map<string, any>): number {
    ...
    for (const [toolKey] of mcpTools) {
      const serverName = toolKey.includes(':') ? toolKey.split(':', 2)[0] : 'unknown'
  ...
  private async executeWithTimeout(
    ...
    return new Promise<ToolResult>((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Tool execution timed out after ${this.config.executionTimeout}ms`))
      ...
      if (schema && typeof schema === 'object' && 'parse' in schema) {
        ...
        if (jsonSchema.type === 'object' && jsonSchema.properties) {
          for (const [key, prop] of Object.entries(jsonSchema.properties)) {
            if (jsonSchema.required && jsonSchema.required.includes(key) && !(key in args)) {
