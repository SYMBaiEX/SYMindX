...
export class ModuleLifecycleManager {
  ...
  private listeners = new Map<
    keyof ModuleLifecycleEvents,
    Set<ModuleLifecycleListener<any>>
  >();
  ...
  registerFactory(
    ...
    runtimeLogger.info(
      `Registered module factory: ${moduleId} v${moduleInfo.version}`
    );
  }
  ...
  async unloadModule(moduleId: string): Promise<void> {
    ...
    if (dependents && dependents.size > 0) {
      ...
      runtimeLogger.warn(
        `Module ${moduleId} has dependents: ${dependentList.join(', ')}`
      );
  ...
  async hotReload(moduleId: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        runtimeLogger.error(
          `Failed to recover module ${moduleId}:`,
          recoveryError
        );
      }
  ...
  async restartModule(moduleId: string): Promise<void> {
    ...
    if (moduleInstance.restartCount >= (this.config.maxRestarts || 3)) {
      throw new Error(
        `Maximum restart attempts exceeded for module: ${moduleId}`
      );
    }
    ...
    for (const depId of dependencies) {
      ...
      if (!depInstance) {
        ...
        try {
          ...
          throw new Error(
            `Failed to resolve dependency ${depId} for module ${moduleId}`
          );
        }
      } else if (depInstance.info.state !== ModuleState.RUNNING) {
        throw new Error(
          `Dependency ${depId} is not running (state: ${depInstance.info.state})`
        );
      }
    }
  }
      ...
      for (const [moduleId] of this.modules) {
        try {
          ...
          if (health.status === 'unhealthy') {
            ...
            if (
              moduleInstance.errorCount >= 3 &&
              moduleInstance.restartCount < (this.config.maxRestarts || 3)
            ) {
              runtimeLogger.warn(
                `Module ${moduleId} failing health checks, attempting restart`
              );
              try {
                ...
                runtimeLogger.error(
                  `Failed to restart unhealthy module ${moduleId}:`,
                  error
                );
              }
            }
          ...
          runtimeLogger.error(
            `Health check error for module ${moduleId}:`,
            error
          );
        }
      }
    ...
    for (const listener of eventListeners) {
      try {
        ...
        if (result instanceof Promise) {
          result.catch((error) => {
            runtimeLogger.error(
              `Error in lifecycle event listener for ${event}:`,
              error
            );
          });
        }
