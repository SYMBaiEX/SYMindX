...
export abstract class SharedMemoryPool<TStorage> {
  ...
  protected accessLog: Array<{ agentId: string; memoryId: string; timestamp: Date; action: string }> = [];
  ...
  async share(
    ...
    if (this.config.maxPoolSize && this.entries.size >= this.config.maxPoolSize) {
  ...
  async unshare(agentId: string, memoryId: string): Promise<void> {
    ...
    if (entry && (entry.agentId === agentId || await this.hasAdminPermission(agentId))) {
   ...
   */
  getStats(): PoolStats {
    const agents = new Set(Array.from(this.entries.values()).map(e => e.agentId));
    const totalAccess = Array.from(this.entries.values()).reduce((sum, e) => sum + e.accessCount, 0);
    const lastActivity = Array.from(this.entries.values()).reduce((latest, e) => 
      e.lastAccessedAt > latest ? e.lastAccessedAt : latest, new Date(0));
    
    const poolSizeBytes = Array.from(this.entries.values()).reduce((size, e) => 
      size + JSON.stringify(e.memory).length, 0);
    
    return {
      poolId: this.poolId,
      totalMemories: this.entries.size,
      sharedBy: Array.from(this.entries.values()).filter(e => e.agentId).length,
      totalAgents: agents.size,
      averageAccessCount: this.entries.size > 0 ? totalAccess / this.entries.size : 0,
      lastActivity,
      poolSizeBytes
    };
  }
   ...
   * Get access history
   */
  getAccessHistory(agentId?: string, limit: number = 100): Array<{
    agentId: string;
    memoryId: string;
    timestamp: Date;
    action: string;
  ...
  async merge(otherPool: SharedMemoryPool<TStorage>): Promise<void> {
    ...
    for (const [key, entry] of otherPool.entries) {
      if (!this.entries.has(key)) {
        ...
        const existing = this.entries.get(key)!;
        if (this.config.enableConflictResolution) {
          const resolved = await this.resolveConflict(existing, entry.memory, entry.agentId);
          existing.memory = resolved;
          existing.version += 1;
        }
      }
    ...
    console.log(`Merged pool ${otherStats.poolId} with ${otherStats.totalMemories} memories`);
  }
   ...
   */
  protected hasPermissions(entry: PoolEntry, requiredPermissions: MemoryPermission[]): boolean {
    if (!this.config.enablePermissions) return true;
    
    return requiredPermissions.every(perm => entry.permissions.includes(perm));
  }
  
  /**
   * Validate agent permissions
   */
  protected async validatePermissions(agentId: string, permissions: MemoryPermission[]): Promise<void> {
    ...
    if (!agentPerms) {
      ...
      this.agentPermissions.set(agentId, new Set([MemoryPermission.READ, MemoryPermission.WRITE]));
      return;
    }
    
    const hasAllPermissions = permissions.every(perm => agentPerms.has(perm));
    if (!hasAllPermissions) {
      throw new Error(`Agent ${agentId} lacks required permissions: ${permissions.join(', ')}`);
    }
  }
   ...
   */
  protected calculateRelevanceScore(memory: MemoryRecord, query: string): number {
   ...
   */
  protected sortByRelevance(memories: MemoryRecord[], agentId: string): MemoryRecord[] {
  ...
  protected async resolveConflict(
    ...
    if (newImportance === existingImportance) {
      return newMemory.timestamp > existing.memory.timestamp ? newMemory : existing.memory;
    }
