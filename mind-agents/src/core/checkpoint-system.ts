...
export class CheckpointSystem extends EventEmitter {
  ...
  async createCheckpoint(
    ...
    try {
      this.logger.info(`Creating ${type} checkpoint for agent ${agent.id}`, { reason })
      ...
      this.updateMetrics(agent.id, true, duration, JSON.stringify(snapshot).length)
      ...
      this.emit('checkpoint_created', { agentId: agent.id, type, filepath, duration })
      ...
      if (schedule) {
        ...
        if (schedule.failures >= schedule.maxFailures) {
          ...
          this.logger.error(`Disabled checkpoints for agent ${agent.id} due to too many failures`)
        }
      }
      
      this.logger.error(`Checkpoint creation failed for agent ${agent.id}:`, error)
   ...
   */
  async createIncrementalCheckpointIfNeeded(agent: Agent): Promise<string | null> {
    ...
    if (!previousHash || this.hasSignificantChanges(currentHash, previousHash)) {
      return await this.createCheckpoint(agent, CheckpointType.INCREMENTAL, 'state_change_detected')
    }
    ...
    for (const [agentId, schedule] of this.schedules) {
      ...
      try {
        ...
        if (schedule.failures >= schedule.maxFailures) {
          ...
          // Retry with delay
          schedule.nextCheckpoint = new Date(Date.now() + this.config.retryDelay)
        }
      }
    }
    ...
    for (const agentId of this.schedules.keys()) {
      try {
        ...
        if (checkpoints.length > 10) {
          ...
          this.logger.debug(`Cleaned up ${toDelete.length} old checkpoints for agent ${agentId}`)
        }
  ...
  private updateMetrics(agentId: string, success: boolean, duration: number, size: number): void {
    ...
    if (success) {
      ...
      const totalTime = metrics.averageCheckpointTime * (metrics.successfulCheckpoints - 1) + duration
  ...
  private hasSignificantChanges(currentHash: string, previousHash: string): boolean {
  ...
  private checkChangeThreshold(agent: Agent, changeCount: number): void {
    if (changeCount >= this.config.stateChangeThreshold) {
      this.createIncrementalCheckpointIfNeeded(agent).catch(error => {
        this.logger.error(`Event-based checkpoint failed for agent ${agent.id}:`, error)
      })
    }
  }

  private calculateSuccessRate(metrics: CheckpointMetrics[]): number {
    const totalCheckpoints = metrics.reduce((sum, m) => sum + m.totalCheckpoints, 0)
    const successfulCheckpoints = metrics.reduce((sum, m) => sum + m.successfulCheckpoints, 0)
