...
export class MCPIntegration {
  ...
  private async connectToServer(serverName: string, client: any, config: MCPServerConfig): Promise<void> {
    try {
      ...
      if (config.retryAttempts && config.retryAttempts > 0) {
        setTimeout(() => {
          this.retryConnection(serverName, client, config, config.retryAttempts! - 1)
        }, 5000)
      }
   ...
   */
  private async retryConnection(serverName: string, client: any, config: MCPServerConfig, attemptsLeft: number): Promise<void> {
    if (attemptsLeft <= 0) {
      runtimeLogger.error(`❌ Failed to connect to MCP server ${serverName} after all retry attempts`)
      return
    }

    try {
      await this.connectToServer(serverName, client, { ...config, retryAttempts: attemptsLeft })
    } catch (error) {
      runtimeLogger.warn(`⚠️ Retry attempt failed for MCP server ${serverName}, ${attemptsLeft} attempts remaining`)
    }
  }
   ...
   */
  private setupAutoReconnect(serverName: string, client: any, config: MCPServerConfig): void {
    client.on('disconnect', () => {
      runtimeLogger.warn(`🔌 MCP server ${serverName} disconnected, attempting to reconnect...`)
      
      const timer = setTimeout(async () => {
        try {
          await this.connectToServer(serverName, client, config)
        } catch (error) {
          runtimeLogger.error(`❌ Auto-reconnect failed for MCP server ${serverName}:`, error)
        }
      }, 5000)
    ...
    try {
      ...
      if (response.tools) {
        for (const tool of response.tools) {
          ...
          runtimeLogger.debug(`🔧 Discovered tool: ${tool.name} from ${serverName}`)
        }
      }
    } catch (error) {
      runtimeLogger.error(`❌ Failed to discover tools from ${serverName}:`, error)
    }
  }
    ...
    try {
      ...
      if (response.resources) {
        for (const resource of response.resources) {
          ...
          runtimeLogger.debug(`📁 Discovered resource: ${resource.name} from ${serverName}`)
        }
      }
    } catch (error) {
      runtimeLogger.error(`❌ Failed to discover resources from ${serverName}:`, error)
    }
  }
    ...
    try {
      ...
      if (response.prompts) {
        for (const prompt of response.prompts) {
          ...
          runtimeLogger.debug(`💭 Discovered prompt: ${prompt.name} from ${serverName}`)
        }
      }
    } catch (error) {
      runtimeLogger.error(`❌ Failed to discover prompts from ${serverName}:`, error)
    }
  }
