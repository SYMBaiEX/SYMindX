...
export class ContextDebugScenarios {
  ...
  async getScenarioRecommendations(contextId?: string): Promise<{
    ...
    if (state.resources.memoryUsage > 0.8) {
      const memoryScenarios = Array.from(this.scenarios.values()).filter(
        (s) => s.category === 'memory'
      );
      recommended.push(...memoryScenarios);
      memoryScenarios.forEach((s) => {
        reasons[s.id] =
          `High memory usage detected (${(state.resources.memoryUsage * 100).toFixed(1)}%)`;
      });
    }

    if (state.resources.overhead > 50) {
      const performanceScenarios = Array.from(this.scenarios.values()).filter(
        (s) => s.category === 'performance'
      );
      recommended.push(...performanceScenarios);
      performanceScenarios.forEach((s) => {
        reasons[s.id] =
          `High observability overhead detected (${state.resources.overhead}ms)`;
      });
    }

    if (state.health.status !== 'healthy') {
      const qualityScenarios = Array.from(this.scenarios.values()).filter(
    ...
    if (contextId && metrics.context?.instances) {
      ...
      if (contextMetrics && contextMetrics.qualityScore < 0.6) {
        const transformationScenarios = Array.from(
        ...
        transformationScenarios.forEach((s) => {
          reasons[s.id] =
            `Low context quality score: ${(contextMetrics.qualityScore * 100).toFixed(1)}%`;
        });
      }
    }
  ...
  private initializeScenarios(): void {
    ...
    this.scenarios.set('performance-bottleneck', {
      ...
      description:
        'Identify and analyze performance bottlenecks in context operations',
      ...
      steps: [
        {
          ...
          description:
            'Begin tracing context operations to capture performance data',
        ...
        },
        {
          ...
          description:
            'Execute multiple context operations to generate performance data',
          code: `
for (let i = 0; i < 100; i++) {
  await contextOperation(contextId, 'test-load-' + i);
      ...
      expectedOutcome:
        'Performance bottlenecks identified with specific optimization recommendations',
      troubleshootingTips: [
    ...
    this.scenarios.set('memory-leak', {
      ...
      description:
        'Detect and analyze potential memory leaks in context operations',
      category: 'memory',
      severity: 'critical',
      steps: [
        {
...
for (let i = 0; i < 1000; i++) {
  ...
  // Context should be cleaned up here
        ...
        },
        {
          ...
          expectedResult:
            'Memory profile analyzed, leaks identified if present',
          debugCommands: [
  ...
  [contextId], 'flow'
          ...
          debugCommands: [
            ...
            'console.log(visualization.edges)',
          ],
        },
        {
          stepNumber: 4,
      ...
      expectedOutcome:
        'Context flow patterns understood with bottlenecks identified',
      troubleshootingTips: [
...
console.log('Quality scores:', trace.quality);`,
      ...
      expectedOutcome:
        'Context quality assessed with improvement recommendations',
      troubleshootingTips: [
  ...
  contextId, version1.version
    ...
    runtimeLogger.debug('Debug scenarios initialized', {
      ...
      categories: Array.from(
        new Set(Array.from(this.scenarios.values()).map((s) => s.category))
      ),
    });
  }
  ...
  private async executeStep(
    ...
    switch (step.action) {
      ...
      case 'Analyze Memory Profile':
        const sessionId =
          await this.observabilitySystem.debugger.startDebugSession(contextId);
        const debugInfo =
          await this.observabilitySystem.debugger.getDebugInfo(sessionId);
        return debugInfo.profile;

      case 'Generate Flow Visualization':
        return await this.observabilitySystem.visualizer.generateVisualization(
          [contextId],
          'flow'
        );
      ...
      case 'Create Initial Version':
        return await this.observabilitySystem.versioning.createVersion(
          contextId,
          'Debug scenario - Initial state',
          ['debug', 'baseline']
        );

      case 'Test Rollback':
        const versions =
          await this.observabilitySystem.versioning.getVersionHistory(
            contextId
          );
        if (versions.length > 0) {
          await this.observabilitySystem.versioning.rollbackToVersion(
            contextId,
            versions[0].version
          );
  ...
  exportScenarioResults(
    ...
    const exportData = {
      ...
      execution: {
        ...
        results,
        summary: this.generateScenarioSummary(
          scenario,
          results,
 ...
 */
export async function runExampleDebugSession(
  observabilitySystem: ContextObservabilitySystem
): Promise<void> {
  ...
  try {
    ...
    // Export results
    const exportData = debugScenarios.exportScenarioResults(
      'performance-bottleneck',
      result.results
    );
    console.log('\n💾 Exported Results:');
