 ...
 */
export class ContextDebuggerImpl extends EventEmitter implements ContextDebugger {
  ...
  async setBreakpoint(
    sessionId: string, 
    breakpoint: Omit<ContextBreakpoint, 'breakpointId' | 'hitCount' | 'createdAt'>
  ): Promise<string> {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new Error(`Debug session not found: ${sessionId}`);
  ...
  async getDebugInfo(sessionId: string): Promise<ContextDebugInfo> {
    ...
    const currentPosition = session.callStack.length > 0 ? {
      ...
      stackTrace: session.callStack.map(frame => 
  ...
  hitBreakpoint(
    ...
    const matchingBreakpoints = Array.from(session.breakpoints.values()).filter(bp => {
      if (bp.type === 'line' && bp.location.moduleId === moduleId && 
          bp.location.operation === operation && bp.location.line === line) {
        return bp.enabled;
      }
  ...
  recordPerformanceEnd(contextId: string, operation: string): void {
    ...
    const perfData = contextPerf.find(p => p.operation === operation && !p.endTime);
    if (!perfData) return;
  ...
  private generateHotSpots(contextId: string): ContextHotSpot[] {
    ...
    contextPerf.forEach(perf => {
      if (perf.endTime) {
        ...
        const existing = operationStats.get(perf.operation) || { count: 0, totalTime: 0 };
    ...
    const totalTime = Array.from(operationStats.values())
  ...
  private generateMemoryProfile(contextId: string): ContextMemoryProfile {
    ...
    const allocations = contextPerf
      .filter(perf => perf.memoryAfter && perf.memoryAfter > perf.memoryBefore)
      .map(perf => ({
    ...
    const totalAllocated = allocations.reduce((sum, alloc) => sum + alloc.size, 0);
    const currentUsage = process.memoryUsage().heapUsed;
    const peakUsage = Math.max(...contextPerf.map(p => p.memoryAfter || p.memoryBefore));
  ...
  private createEvaluationContext(session: DebugSession): Record<string, unknown> {
  ...
  private isExpressionSafe(expression: string): boolean {
    ...
    const dangerous = [
      ...
      '__proto__', 'constructor', 'prototype', 'delete', 'with'
    ];
  ...
  private generateOptimizationSuggestions(operation: string, stats: { count: number; totalTime: number }): string[] {
  ...
  getStatistics() {
    const activeSessions = Array.from(this.sessions.values())
      .filter(s => s.state !== 'stopped').length;

    return {
      totalSessions: this.sessions.size,
      activeSessions,
      totalBreakpoints: Array.from(this.sessions.values())
        .reduce((sum, s) => sum + s.breakpoints.size, 0),
      performanceDataPoints: Array.from(this.performanceData.values())
