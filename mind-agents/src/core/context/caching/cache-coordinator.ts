...
export class CacheCoordinator implements ContextCacheManager {
  ...
  async set<T>(key: string, value: T, options: CacheSetOptions = {}): Promise<OperationResult> {
    ...
    try {
      ...
      if (result.success) {
        ...
        if ((targetLevel === CacheLevel.L1 || targetLevel === CacheLevel.L2) && this.config.l3.enabled) {
  ...
  async getMetrics(): Promise<CacheMetrics> {
    ...
    const aggregated: CacheMetrics = {
      hitRate: {
        ...
        overall: this.calculateOverallHitRate([l1Metrics, l2Metrics, l3Metrics]),
      },
      responseTime: {
        l1: l1Metrics.responseTime?.l1 ?? 0,
        l2: l2Metrics.responseTime?.l2 ?? 0,
        l3: l3Metrics.responseTime?.l3 ?? 0,
        average: this.calculateAverageResponseTime([l1Metrics, l2Metrics, l3Metrics]),
      },
      memoryUsage: {
        l1: l1Metrics.memoryUsage?.l1 ?? 0,
        l2: l2Metrics.memoryUsage?.l2 ?? 0,
        total: (l1Metrics.memoryUsage?.l1 ?? 0) + (l2Metrics.memoryUsage?.l2 ?? 0),
      ...
      entries: {
        ...
        total: (l1Metrics.entries?.l1 ?? 0) + (l2Metrics.entries?.l2 ?? 0) + (l3Metrics.entries?.l3 ?? 0),
      },
      operations: {
        reads: (l1Metrics.operations?.reads ?? 0) + (l2Metrics.operations?.reads ?? 0) + (l3Metrics.operations?.reads ?? 0),
        writes: (l1Metrics.operations?.writes ?? 0) + (l2Metrics.operations?.writes ?? 0) + (l3Metrics.operations?.writes ?? 0),
        evictions: (l1Metrics.operations?.evictions ?? 0) + (l2Metrics.operations?.evictions ?? 0) + (l3Metrics.operations?.evictions ?? 0),
        invalidations: (l1Metrics.operations?.invalidations ?? 0) + (l2Metrics.operations?.invalidations ?? 0) + (l3Metrics.operations?.invalidations ?? 0),
      },
      errors: {
    ...
    try {
      if (level) {
        ...
        return {
          ...
          timestamp: Date.now(),
          duration: performance.now() - startTime,
          error: success ? undefined : 'Failed to clear one or more cache levels',
          data: {
   ...
   */
  async invalidate(criteria: CacheInvalidationCriteria): Promise<OperationResult> {
    if (!this.isInitialized) {
      return {
        success: false,
        error: 'Cache coordinator not initialized',
  ...
  async optimize(): Promise<OperationResult> {
    ...
    try {
      ...
      return {
        ...
        timestamp: Date.now(),
        duration: performance.now() - startTime,
        error: success ? undefined : 'Failed to optimize one or more cache levels',
        data: {
  ...
  async shutdown(): Promise<OperationResult> {
    ...
    try {
      ...
      return {
        ...
        timestamp: Date.now(),
        duration: performance.now() - startTime,
        error: success ? undefined : 'Failed to shutdown one or more cache levels',
        data: {
   ...
   */
  async import(data: CacheImportData, options?: CacheImportOptions): Promise<OperationResult> {
  ...
  private async determineCachePlacement<T>(
    ...
    if (this.cacheStrategy) {
      const level = this.cacheStrategy.determineCacheLevel(key, value, options.metadata ?? {});
  ...
  private async considerPromotion<T>(
    ...
    if (toLevel === CacheLevel.L1 && size > this.config.l1.maxMemoryBytes / 10) {
      return;
    }
    
    if (toLevel === CacheLevel.L2 && size > this.config.l2.maxMemoryBytes / 10) {
      return;
    }
   ...
   */
  private calculateOverallHitRate(metricsArray: Partial<CacheMetrics>[]): number {
    let totalHits = 0;
    let totalRequests = 0;
   ...
   */
  private calculateAverageResponseTime(metricsArray: Partial<CacheMetrics>[]): number {
    let totalTime = 0;
    let count = 0;
    
    for (const metrics of metricsArray) {
      if (metrics.responseTime) {
        if (metrics.responseTime.l1 > 0) { totalTime += metrics.responseTime.l1; count++; }
        if (metrics.responseTime.l2 > 0) { totalTime += metrics.responseTime.l2; count++; }
        if (metrics.responseTime.l3 > 0) { totalTime += metrics.responseTime.l3; count++; }
      }
    }
   ...
   */
  private calculateMemoryPercentage(metricsArray: Partial<CacheMetrics>[]): number {
