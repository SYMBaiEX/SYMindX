...
export class PredictiveStrategy implements CacheStrategyInterface {
  ...
  handleMemoryPressure(
    ...
    criteria.push({
      filter: (entry) => {
        ...
        return (
          estimatedAccessTime > this.config.predictionHorizon * 2 &&
  ...
  private predictContextualAccess(key: string, context: Metadata): number {
    ...
    for (const [patternKey, pattern] of this.accessPatterns) {
      ...
      if (similarity > 0.5) {
        ...
        const coOccurrence = this.calculateCoOccurrenceProbability(
          patternKey,
          key
        );
  ...
  private predictSequentialAccess(key: string): number {
    ...
    for (const pattern of this.sequentialPatterns) {
      ...
      if (matchLength > 0) {
        ...
        const probability =
          (matchLength / pattern.sequence.length) *
          nextKeyProb *
          pattern.confidence;
        maxProbability = Math.max(maxProbability, probability);
      }
  ...
  private updateTemporalPattern(key: string, timestamp: number): void {
    ...
    pattern.hourPattern[hour] =
      pattern.hourPattern[hour] * (1 - this.config.learningRate) +
      this.config.learningRate;

    // Update day pattern with learning rate
    pattern.dayPattern[day] =
      pattern.dayPattern[day] * (1 - this.config.learningRate) +
      this.config.learningRate;
    ...
    if (recentAccesses.length >= 3) {
      const oldAccesses = recentAccesses.slice(
        0,
        Math.floor(recentAccesses.length / 2)
      );
      const newAccesses = recentAccesses.slice(
        Math.floor(recentAccesses.length / 2)
      );

      const oldRate =
        oldAccesses.length /
        ((oldAccesses[oldAccesses.length - 1]?.timestamp || 0) -
          (oldAccesses[0]?.timestamp || 0));
      const newRate =
        newAccesses.length /
        ((newAccesses[newAccesses.length - 1]?.timestamp || 0) -
          (newAccesses[0]?.timestamp || 0));
  ...
  private updateSequentialPatterns(currentKey: string): void {
    ...
    let existingPattern = this.sequentialPatterns.find(
      (p) =>
    ...
    const totalNext = Array.from(existingPattern.nextKeys.values()).reduce(
      (sum, count) => sum + count,
      0
    );
    ...
    if (this.sequentialPatterns.length > this.config.maxPatterns / 10) {
      ...
      this.sequentialPatterns = this.sequentialPatterns.slice(
        0,
        this.config.maxPatterns / 10
      );
    }
  }
  ...
  private calculateCoOccurrenceProbability(key1: string, key2: string): number {
    ...
    const key1Accesses = this.recentAccesses.filter(
      (access) => access.key === key1
    );
    ...
    for (const access1 of key1Accesses) {
      ...
      const hasKey2 = this.recentAccesses.some(
        (access2) =>
          access2.key === key2 &&
