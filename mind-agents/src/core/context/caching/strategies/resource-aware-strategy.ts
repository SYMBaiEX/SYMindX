...
export class ResourceAwareStrategy implements CacheStrategyInterface {
   ...
   */
  async initialize(config?: Partial<ResourceAwareConfig>): Promise<OperationResult> {
  ...
  determineCacheLevel(key: string, data: unknown, metadata: Metadata): CacheLevel {
    ...
    if (preferredLevel === CacheLevel.L1 && dataSize > 10240) { // 10KB
      return CacheLevel.L2;
    }
    
    if (preferredLevel === CacheLevel.L2 && dataSize > 102400) { // 100KB
      return CacheLevel.L3;
    }
  ...
  calculatePriority(entry: CacheEntry): number {
    ...
    if (sizeRatio > 0.5) {
      resourceAdjustedPriority *= (1 - (sizeRatio - 0.5) * adjustments.evictionAggressiveness);
    }
    
    // Boost priority for recently accessed items under high pressure
    if (this.currentPressure === ResourcePressure.HIGH || this.currentPressure === ResourcePressure.CRITICAL) {
  ...
  recommendWarming(context: Metadata): string[] {
    ...
    if (this.currentPressure === ResourcePressure.HIGH || 
        this.currentPressure === ResourcePressure.CRITICAL) {
      return [];
    }
    
    // Under medium pressure, recommend only high-confidence items
    const maxRecommendations = this.currentPressure === ResourcePressure.MEDIUM ? 5 : 20;
  ...
  handleMemoryPressure(currentUsage: number, threshold: number): CacheInvalidationCriteria[] {
    ...
    if (pressureRatio > 0.9) {
      ...
      criteria.push({
        filter: (entry) => {
          ...
          return entry.size > sizeThreshold || entry.lastAccessedAt < ageThreshold;
        },
      });
    }
    ...
    if (trend.memoryTrend > 0.1) { // Memory usage increasing
      criteria.push({
        filter: (entry) => {
  ...
  private async measureResources(): Promise<void> {
    try {
      ...
      if (this.resourceHistory.length > this.config.sampleHistorySize) {
        this.resourceHistory = this.resourceHistory.slice(-this.config.sampleHistorySize);
      }
    } catch (error) {
  ...
  private updateResourcePressure(): void {
    ...
    if (latest.ioLatency > this.config.ioLatencyThreshold) {
      pressureScore += (latest.ioLatency / this.config.ioLatencyThreshold - 1) * 1;
    }
