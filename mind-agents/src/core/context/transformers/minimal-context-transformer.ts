...
class ContextCompressor {
  ...
  static generateSummary(text: string, maxLength: number = 100): string {
    ...
    for (const sentence of sentences) {
      ...
      const uniqueWords = words.filter(w => !this.COMMON_WORDS.has(w) && w.length > 2);
  ...
  static extractFlags(context: UnifiedContext): string[] {
    ...
    if (context.messages.some(m => m.emotions?.length)) flags.push('emotional');
 ...
 */
export class MinimalContextTransformer implements ContextTransformer<UnifiedContext, MinimalContext> {
  ...
  private transformationCache = new Map<string, TransformationResult<MinimalContext>>();
  ...
  async transform(
    ...
    try {
      ...
      if (this.transformationCache.has(cacheKey)) {
        ...
        runtimeLogger.debug(`Cache hit for minimal transformation: ${cacheKey}`);
        return {
          ...cached,
          cached: true
        };
      ...
      runtimeLogger.debug(`Minimal context transformation completed in ${duration.toFixed(2)}ms`);
      return result;

    } catch (error) {
      const duration = performance.now() - startTime;
      runtimeLogger.error('Minimal context transformation failed', { error, duration });
  ...
  private calculatePriority(context: UnifiedContext): number {
    ...
    if (context.emotionData?.currentEmotions?.length) {
      const avgIntensity = context.emotionData.currentEmotions.reduce(
   ...
   */
  private applyMinimalStrategy(baseContext: MinimalContext, original: UnifiedContext): MinimalContext {
    return {
      id: baseContext.id,
   ...
   */
  private applyOptimizedStrategy(baseContext: MinimalContext, original: UnifiedContext): MinimalContext {
   ...
   */
  private applyCachedStrategy(baseContext: MinimalContext, original: UnifiedContext): MinimalContext {
   ...
   * Cache management
   */
  private cacheResult(key: string, result: TransformationResult<MinimalContext>): void {
  ...
  async validate(
    ...
    const score = errors.length === 0 ? (warnings.length === 0 ? 1.0 : 0.85) : 0.4;
   ...
   */
  static isStale(context: MinimalContext, thresholdSeconds: number = 3600): boolean {
  ...
  private getTransformedFields(strategy: TransformationStrategy): string[] {
    const baseFields = ['id', 'agentId', 'timestamp', 'content', 'phase', 'priority', 'meta'];
  ...
  private getDroppedFields(strategy: TransformationStrategy): string[] {
    ...
    return [
      'messages', 'state', 'environment', 'metadata', 'cognitionData',
      'emotionData', 'memoryData', 'extensionData', 'performance'
    ];
  }
...
export class MinimalContextUtils {
  ...
  static toBasicUnified(minimal: MinimalContext): Partial<UnifiedContext> {
    return {
      ...
      environment: {
        capabilities: minimal.meta.flags.includes('capable') ? ['basic'] : undefined,
        constraints: minimal.meta.flags.includes('constrained') ? ['limited'] : undefined
      },
      metadata: {
