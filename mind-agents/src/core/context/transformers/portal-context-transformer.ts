 ...
 */
export class PortalContextTransformer
  implements ContextTransformer<UnifiedContext, EnhancedPortalContext>
{
  ...
  private transformationCache = new Map<
    string,
    TransformationResult<EnhancedPortalContext>
  >();
  ...
  async transform(
    ...
    try {
      ...
      if (config?.cache?.enabled) {
        ...
        if (cached) {
          runtimeLogger.debug(
            `Cache hit for portal transformation: ${cacheKey}`
          );
          return {
            ...cached,
            cached: true,
      ...
      runtimeLogger.debug(
        `Portal context transformation completed in ${duration.toFixed(2)}ms`
      );
      return result;
    } catch (error) {
      const duration = performance.now() - startTime;
      runtimeLogger.error('Portal context transformation failed', {
        error,
        duration,
      });
  ...
  private generateSystemPrompt(context: UnifiedContext): string {
    const personality =
      context.extensionData?.['personality'] || 'helpful assistant';
  ...
  private generateContentSummary(context: UnifiedContext): string {
    ...
    return recentMessages
      .map((m) => `${m.from}: ${m.content.slice(0, 50)}`)
      .join('; ');
  }
  ...
  private extractKeyPoints(context: UnifiedContext): string[] {
    ...
    if (context.environment.capabilities?.length) {
      points.push(
        `Capabilities: ${context.environment.capabilities.slice(0, 3).join(', ')}`
      );
    }
  ...
  private optimizeMessageHistory(context: UnifiedContext): OptimizedMessage[] {
    return context.messages.map((msg, index) => ({
      ...
      importance: this.calculateMessageImportance(
        msg,
        index,
  ...
  private determineConversationTone(
    ...
    if (
      messages.some(
        (m) => m.content.includes('please') || m.content.includes('thank you')
      )
    ) {
      return 'professional';
    }
   ...
   */
  private generateModelInstructions(
    context: UnifiedContext
  ): ModelInstruction[] {
    ...
    if (context.state.complexity > 0.7) {
      instructions.push({
        type: 'behavior',
        instruction:
          'This is a complex topic. Break down your response into clear, digestible parts.',
        priority: 0.9,
      });
    }
   ...
   */
  private generateResponseConstraints(
    context: UnifiedContext
  ): ResponseConstraint[] {
  ...
  private determineOutputFormat(context: UnifiedContext): OutputFormat {
    ...
    if (
      ...
      lastMessage?.content.includes('structured')
    ) {
      return {
        type: 'json',
  ...
  private applyStreamingStrategy(
    context: EnhancedPortalContext
  ): EnhancedPortalContext {
    return {
      ...context,
  ...
  private extractRelevantHistory(context: UnifiedContext): HistoryEntry[] {
    return context.messages.slice(-10).map((msg, index) => ({
      ...
      content: msg.content,
      timestamp: msg.timestamp,
      relevance: this.calculateMessageImportance(
        msg,
        index,
        context.messages.length
      ),
  ...
  private extractContextualMemories(
    context: UnifiedContext
  ): ContextualMemory[] {
  ...
  private generateTokenOptimization(
    context: UnifiedContext
  ): TokenOptimization {
    const contentLength =
  ...
  private determineExpectedLength(
    context: UnifiedContext
  ): 'brief' | 'moderate' | 'detailed' | 'comprehensive' {
    const complexity = context.state.complexity;
    const messageLength =
      context.messages.length > 0
        ? context.messages[context.messages.length - 1].content.length
        : 0;
  ...
  private extractMultiModalContext(
    context: UnifiedContext
  ): MultiModalContext | undefined {
    // Check if context contains multimodal elements
    const hasMultiModal = context.messages.some(
      (m) =>
        m.metadata &&
        (m.metadata.images || m.metadata.audio || m.metadata.video)
    );
  ...
  private generateErrorRecoveryStrategy(
    context: UnifiedContext
  ): ErrorRecoveryStrategy {
    return {
      fallbackResponse:
        'I apologize, but I encountered an issue processing your request. Could you please rephrase or provide more context?',
      retryInstructions: [
      ...
      userNotification:
        "I'm having trouble understanding. Let me try a different approach.",
    };
  }
