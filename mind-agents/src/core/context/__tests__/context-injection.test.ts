...
describe('Context Injection and Dependency Management', () => {
  ...
  describe('Context Dependency Injection', () => {
    ...
    it('should provide context data to agent event processing', async () => {
      ...
      agent.processEvent.mockImplementation(async (event) => {
        ...
        expect(contextSummary!.pendingQuestions).toContain(
          'What is your favorite programming language?'
        );
    ...
    it('should inject context into memory operations', async () => {
      ...
      contextManager.addMessage(
        context,
        agent.id,
        'I can help you with useState and useEffect'
      );
      ...
      const preservedMemory = await contextManager.preserveToMemory(
        agent,
        context.id
      );
    ...
    it('should inject context into extension methods', async () => {
      ...
      const result = await mockExtension.actions.processMessage({
        message: 'Test',
      });
  ...
  describe('Context Scope Management', () => {
    it('should maintain proper context scoping per agent', () => {
      ...
      expect(contextManager.getActiveContext(agent1.id)?.id).not.toBe(
        context2.id
      );
      expect(contextManager.getActiveContext(agent2.id)?.id).not.toBe(
        context1.id
      );
    });
  ...
  describe('Dependency Resolution', () => {
    ...
    it('should inject optional dependencies when available', async () => {
      ...
      if (context.metadata['services']?.sentimentAnalyzer) {
        const sentiment = context.metadata[
          'services'
        ].sentimentAnalyzer.analyze(context.messages[0].content);
  ...
  describe('Context Provider Integration', () => {
    it('should integrate with custom context providers', async () => {
      ...
      // Use custom provider
      const customContext = customProvider.createContext(
        'test-agent-1',
        'user-1'
      );
      const enhancedContext = customProvider.enhanceContext(customContext, {
        version: '1.0',
      });

      expect(enhancedContext.metadata['provider']).toBe('custom');
      expect(enhancedContext.metadata['enhanced']).toBe(true);
      expect(enhancedContext.metadata['enhancementData']).toEqual({
        version: '1.0',
      });
    });
    ...
    it('should validate provider compatibility', () => {
      ...
      const hasOptionalMigration =
        typeof testProvider.migrateContext === 'function';
  ...
  describe('Context Lifecycle Hooks', () => {
    ...
    it('should support conditional hook execution', async () => {
      ...
      await conditionalHooks.onlyForImportantMessages(
        context,
        importantMessage
      );
