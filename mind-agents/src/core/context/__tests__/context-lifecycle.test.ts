...
describe('Context Lifecycle Management', () => {
  ...
  describe('Context Creation Lifecycle', () => {
    ...
    it('should handle context creation with custom configuration', () => {
      ...
      const context = customManager.getOrCreateContext(
        'test-agent-1',
        'user-1'
      );
  ...
  describe('Context Evolution and Updates', () => {
    ...
    it('should track context complexity growth', () => {
      ...
      contextManager.addMessage(
        context,
        'user-2',
        'Also, what about data structures?'
      );
      contextManager.addMessage(
        context,
        'test-agent-1',
      ...
      expect(finalComplexity.messageCount).toBeGreaterThan(
        initialComplexity.messageCount
      );
      expect(finalComplexity.topicCount).toBeGreaterThan(
        initialComplexity.topicCount
      );
      expect(finalComplexity.participantCount).toBeGreaterThan(
        initialComplexity.participantCount
      );
      expect(finalComplexity.questionCount).toBeGreaterThan(
        initialComplexity.questionCount
      );
    });
  });
  ...
  describe('Context Persistence and Recovery', () => {
    ...
    it('should import and restore context state', () => {
      ...
      const originalContext = contextManager.getOrCreateContext(
        'agent-1',
        'user-1'
      );
      ...
      const newManager = createContextManager(
    ...
    it('should handle corrupted context data gracefully', () => {
      ...
      const validContext = contextManager.getOrCreateContext(
        'test-agent',
        'user-1'
      );
  ...
  describe('Context Cleanup and Garbage Collection', () => {
    ...
    it('should preserve important contexts during cleanup', async () => {
      ...
      const importantContext = preservingManager.getOrCreateContext(
        'important-agent',
        'user-1'
      );
      ...
      for (let i = 0; i < 20; i++) {
        ...
        preservingManager.addMessage(
          importantContext,
          'important-agent',
          `Important response ${i}`
        );
      }

      // Create less important context
      const simpleContext = preservingManager.getOrCreateContext(
        'simple-agent',
        'user-2'
      );
    ...
    it('should scale cleanup operations efficiently', async () => {
      ...
      await PerformanceAssertions.assertExecutionTime(
        () => {
          ...
          contextManager.exportContexts().filter(
            (ctx) => Date.now() - ctx.lastActive.getTime() < 5000 // 5 seconds
          );
        },
