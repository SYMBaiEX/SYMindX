...
export class ResourceManager extends EventEmitter {
  ...
  async registerResource(
    ...
    if (agentResourceCount > this.config.maxResourcesPerAgent) {
      this.logger.warn(`Agent ${agentId} has ${agentResourceCount} resources (max: ${this.config.maxResourcesPerAgent})`)
      this.emit('resource_limit_exceeded', { agentId, count: agentResourceCount })
    }
    
    if (this.config.enableResourceLogging) {
      this.logger.debug(`Registered resource ${resourceId}`, { agentId, type, description })
    }
  ...
  async deactivateResource(resourceId: string): Promise<boolean> {
    ...
    if (this.config.enableResourceLogging) {
      this.logger.debug(`Deactivated resource ${resourceId}`, { agentId: resource.agentId, type: resource.type })
    }
    
    this.emit('resource_deactivated', { resourceId, agentId: resource.agentId })
  ...
  async unregisterResource(resourceId: string): Promise<boolean> {
    ...
    try {
      ...
      if (this.config.enableResourceLogging) {
        this.logger.debug(`Unregistered resource ${resourceId}`, { agentId: resource.agentId, type: resource.type })
      }
      
      this.emit('resource_unregistered', { resourceId, agentId: resource.agentId })
      
      return true
      
    } catch (error) {
      this.logger.error(`Failed to cleanup resource ${resourceId}:`, error)
      this.emit('resource_cleanup_failed', { resourceId, agentId: resource.agentId, error })
      return false
    }
  }
  ...
  async cleanupAgentResources(agentId: string): Promise<CleanupResult> {
    ...
    this.logger.info(`Cleaning up ${agentResourceIds.size} resources for agent ${agentId}`)
   ...
   */
  getStaleResources(maxAgeMs: number = this.config.resourceTimeoutMs): ResourceHandle[] {
  ...
  async cleanupStaleResources(): Promise<CleanupResult> {
    ...
    for (const resource of staleResources) {
      try {
        ...
        result.failed++
        result.errors.push(`Failed to cleanup stale resource ${resource.id}: ${error}`)
      }
    }
  ...
  getHealthReport(): ResourceHealthReport {
    ...
    const memoryUsage = this.config.trackMemoryUsage ? process.memoryUsage() : undefined
    ...
    return {
      ...
      resourcesByType,
      memoryUsage,
      health: this.calculateHealthScore(totalResources, staleResources.length, activeResources)
    }
  }
  ...
  private calculateHealthScore(total: number, stale: number, active: number): number {
