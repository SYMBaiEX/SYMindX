...
export class MultiAgentManager extends EventEmitter {
  ...
  async spawnAgent(request: AgentSpawnRequest): Promise<string> {
    ...
    try {
      ...
      const characterConfig = await this.loadCharacterConfig(request.characterId)
      
      // Generate unique agent ID
      const agentId = `agent_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      const instanceName = request.instanceName || `${characterConfig.name}_${agentId.split('_')[2]}`
      ...
      this.emit('agentSpawned', { agentId, characterId: request.characterId, instanceName })
      this.logger.info(`Agent spawned successfully: ${agentId} (${instanceName})`)
  ...
  async restartAgent(agentId: string): Promise<void> {
    ...
    const originalConfig = (agent as any).originalConfig || (agent as any).config
    ...
    try {
      ...
      this.emit('agentRestarted', { oldAgentId: agentId, newAgentId, instanceName })
  ...
  findAgentsBySpecialty(specialty: string): Agent[] {
    return this.getAvailableAgents().filter(agent => {
      ...
      if (originalConfig?.capabilities) {
        return Object.values(originalConfig.capabilities).some((category: any) => 
      ...
      if (personality) {
        ...
        if (relevantTraits[specialty as keyof typeof relevantTraits]) {
          return relevantTraits[specialty as keyof typeof relevantTraits].some(trait => 
            personality[trait] && personality[trait] > 0.7
          )
        }
      }
  ...
  findAgentsByPersonality(traits: string[]): Agent[] {
    return this.getAvailableAgents().filter(agent => {
      ...
      return traits.some(trait => personality[trait] && personality[trait] > 0.7)
    })
  }
  ...
  routeConversation(requirements: ConversationRequirements): Agent | null {
    ...
    if (requirements.excludeAgents) {
      candidates = candidates.filter(agent => 
        !requirements.excludeAgents!.includes(agent.id)
      )
    }
  ...
  private calculateAgentScore(agent: Agent, requirements: ConversationRequirements): number {
    let score = 0
    const originalConfig = (agent as any).originalConfig || (agent as any).config
    ...
    if (requirements.responseStyle && originalConfig?.communication?.style) {
      if (originalConfig.communication.style.includes(requirements.responseStyle)) {
        score += 10
      }
    }
  ...
  getSystemMetrics(): MultiAgentMetrics {
    ...
    return {
      ...
      averageResponseTime: healthData.length > 0 
        ? healthData.reduce((sum, h) => sum + h.averageResponseTime, 0) / healthData.length
        : 0,
  ...
  private calculateSystemLoad(): number {
    ...
    const activeAgents = Array.from(this.agents.values())
      .filter(a => a.status === AgentStatus.ACTIVE || a.status === AgentStatus.THINKING).length
  ...
  async requestAgentConsultation(collaboration: AgentCollaboration): Promise<string> {
    ...
    this.logger.info(`Agent consultation: ${collaboration.consultingAgent} consulting ${collaboration.consultedAgent}`)
  ...
  enableAgentCollaboration(agentIds: string[]): void {
    ...
    this.logger.info(`Enabling collaboration between agents: ${agentIds.join(', ')}`)
  }
  ...
  listAgents(): Array<{id: string, name: string, status: AgentStatus, characterId: string}> {
