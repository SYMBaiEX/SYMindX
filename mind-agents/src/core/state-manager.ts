...
export class StateManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info('State manager initialized', { directory: this.stateDirectory })
    } catch (error) {
   ...
   */
  async createSnapshot(agent: Agent, type: CheckpointType = CheckpointType.FULL): Promise<AgentStateSnapshot> {
    const agentId = agent.id
    
    if (this.operationLocks.get(agentId)) {
      throw new Error(`State operation already in progress for agent ${agentId}`)
    }
      ...
      if (agent.portals) {
        for (const portal of agent.portals) {
          if (typeof (portal as any).getState === 'function') {
            portalStates[(portal as any).id || 'primary'] = await (portal as any).getState()
          }
        }
      }
  ...
  async saveSnapshot(snapshot: AgentStateSnapshot): Promise<string> {
    ...
    try {
      ...
      this.logger.info(`Snapshot saved for agent ${snapshot.agentId}`, { filename })
      ...
      this.logger.error(`Failed to save snapshot for agent ${snapshot.agentId}:`, error)
      throw error
    }
  }
   ...
   */
  async loadSnapshot(agentId: string, specific?: string): Promise<AgentStateSnapshot | null> {
    const checkpointDir = path.join(this.stateDirectory, agentId, 'checkpoints')
   ...
   */
  async validateSnapshot(snapshot: AgentStateSnapshot): Promise<StateValidationReport> {
    const errors: string[] = []
    ...
    try {
      ...
      if (this.config.validationLevel === 'full' || this.config.validationLevel === 'strict') {
      ...
      if (errors.length === 0) {
        result = warnings.length === 0 ? StateValidationResult.VALID : StateValidationResult.RECOVERABLE
      } else {
        result = recoveryOptions.length > 0 ? StateValidationResult.RECOVERABLE : StateValidationResult.CORRUPTED
      }
  ...
  async deleteCheckpoints(agentId: string, specific?: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to delete checkpoints for agent ${agentId}:`, error)
      throw error
    }
  }
  ...
  private async getRecentMemories(agent: Agent): Promise<MemoryRecord[]> {
    try {
      ...
      this.logger.warn(`Failed to get recent memories for agent ${agent.id}:`, error)
      return []
    }
  }
  ...
  private gatherDependencies(agent: Agent): string[] {
    ...
    agent.extensions.forEach(ext => {
      ...
      if (ext.dependencies) {
        dependencies.push(...ext.dependencies.map(dep => `extension-dep:${dep}`))
      }
    })
  ...
  private async cleanupOldCheckpoints(agentId: string): Promise<void> {
    try {
      ...
      if (checkpoints.length > this.config.maxCheckpoints) {
        ...
        this.logger.info(`Cleaned up ${toDelete.length} old checkpoints for agent ${agentId}`)
      }
    } catch (error) {
      this.logger.error(`Failed to cleanup old checkpoints for agent ${agentId}:`, error)
    }
  }
  ...
  private async updateMetadata(agentId: string, metadata: any): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to update metadata for agent ${agentId}:`, error)
    }
  }
}