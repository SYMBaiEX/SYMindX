...
export class MCPResponseFormatter {
  ...
  private static formatContext7Result(
    ...
    if (toolName.includes('resolve-library-id')) {
      const libraries = data as Array<{ id: string; name: string; description: string }>
      if (Array.isArray(libraries) && libraries.length > 0) {
        const lib = libraries[0]
        return `I found the ${lib.name} library that matches your query.`
      }
    }
    ...
    if (toolName.includes('get-library-docs')) {
      ...
      if (typeof data === 'string') {
        ...
        if (!response) {
          const firstMeaningfulContent = lines.find(line => 
            line.length > 50 && 
            !line.startsWith('#') && 
            !line.startsWith('```') &&
      ...
      const docs = data as { content?: string; sections?: Array<{ title: string; content: string }> }
      if (docs.content) {
  ...
  private static extractSection(
    ...
    let content = ''
    for (let i = sectionIndex + 1; i < lines.length && i < sectionIndex + 5; i++) {
      const line = lines[i]
   ...
   */
  private static formatNoResultsResponse(_context: ConversationContext): string {
   ...
   */
  private static formatPartialErrorAcknowledgment(errors: ToolResult[]): string {
    if (errors.length === 1) {
