...
export interface StyleAdapterConfig extends BaseConfig {
  ...
  stylePresets?: Record<string, CommunicationStyle>
}
...
export class StyleAdapter {
   ...
   * Get style preset
   */
  getPreset(name: string): CommunicationStyle | null {
    return this.config.stylePresets?.[name] || null
  }
  
  /**
   * Apply style preset
   */
  applyPreset(userId: string, presetName: string): boolean {
    const preset = this.getPreset(presetName)
    if (!preset) return false
    
    this.userStyles.set(userId, { ...preset })
    return true
  }
