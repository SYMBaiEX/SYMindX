...
import { performanceBenchmark, quickBenchmark } from '../utils/PerformanceBenchmark';
...
describe('Performance Optimization Components', () => {
  ...
  describe('OptimizedEventBus', () => {
    test('should handle high-volume events efficiently', async () => {
      ...
      const benchmark = await quickBenchmark('event-bus-volume', () => {
        for (let i = 0; i < 100; i++) {
          eventBus.emit({
            id: `event-${i}`,
            type: 'test-event',
    ...
    test('should prioritize events correctly', async () => {
      ...
      eventBus.subscribe('priority-event', (event) => {
        receivedEvents.push(event);
      }, { priority: 10 });
  ...
  describe('LRUCache', () => {
    ...
    test('should perform better than Map for cache workloads', async () => {
      ...
      const mapBenchmark = await quickBenchmark('map-cache', () => {
      ...
      const lruBenchmark = await quickBenchmark('lru-cache', () => {
        const key = `key-${Math.floor(Math.random() * 100)}`;
        if (Math.random() > 0.5) {
      ...
      const performanceRatio = lruBenchmark.duration.avg / mapBenchmark.duration.avg;
  ...
  describe('AsyncQueue', () => {
    test('should execute tasks in priority order', async () => {
      ...
      queue.add(() => { executionOrder.push(1); }, { priority: 1 });
      queue.add(() => { executionOrder.push(5); }, { priority: 5 });
      queue.add(() => { executionOrder.push(3); }, { priority: 3 });
      queue.add(() => { executionOrder.push(10); }, { priority: 10 });
  ...
  describe('Integration Performance Tests', () => {
    test('should show overall performance improvement', async () => {
      ...
      for (const [testName, comparison] of results) {
        console.log(`${testName}: ${comparison.verdict} (${comparison.improvement.duration.toFixed(2)}% duration improvement)`);
    ...
    test('should maintain sub-200ms response times for critical paths', async () => {
      ...
      for (const path of criticalPaths) {
        ...
        console.log(`${path.name}: ${result.duration.avg.toFixed(2)}ms average`);
