...
describe('Context Performance Benchmark Suite', () => {
  ...
  describe('Context Operations Baseline Comparison', () => {
    it('should benchmark context creation performance', async () => {
      ...
      console.log(`Context creation - Baseline: ${baselineTime.toFixed(2)}ms, Enhanced: ${enhancedTime.toFixed(2)}ms, Ratio: ${performanceRatio.toFixed(2)}`);
    ...
    it('should benchmark message processing performance', async () => {
      ...
      baselineContexts.forEach((context, i) => {
        for (let j = 0; j < messagesPerContext; j++) {
          baselineManager.addMessage(context, 'user-1', `Baseline message ${i}-${j}`);
        }
      });
      const baselineEnd = performance.now();
      const baselineTime = baselineEnd - baselineStart;
      ...
      enhancedContexts.forEach((context, i) => {
        for (let j = 0; j < messagesPerContext; j++) {
          contextManager.addMessage(context, 'user-1', `Enhanced message ${i}-${j}`);
        }
      });
      const enhancedEnd = performance.now();
      const enhancedTime = enhancedEnd - enhancedStart;
      ...
      const performanceRatio = enhancedTime / baselineTime;
      console.log(`Message processing - Baseline: ${baselineTime.toFixed(2)}ms, Enhanced: ${enhancedTime.toFixed(2)}ms, Ratio: ${performanceRatio.toFixed(2)}`);
    ...
    it('should benchmark context retrieval performance', async () => {
      ...
      // Setup contexts
      const baselineContexts = Array.from({ length: iterations }, (_, i) => {
        const context = baselineManager.getOrCreateContext(`baseline-agent-${i}`, 'user-1');
        baselineManager.addMessage(context, 'user-1', `Setup message ${i}`);
        return context.id;
      });
      
      const enhancedContexts = Array.from({ length: iterations }, (_, i) => {
        const context = contextManager.getOrCreateContext(`enhanced-agent-${i}`, 'user-1');
      ...
      const performanceRatio = enhancedTime / baselineTime;
      console.log(`Context retrieval - Baseline: ${baselineTime.toFixed(2)}ms, Enhanced: ${enhancedTime.toFixed(2)}ms, Ratio: ${performanceRatio.toFixed(2)}`);
    ...
    it('should benchmark context enrichment performance', async () => {
      ...
      const contexts = Array.from({ length: iterations }, (_, i) => {
        const context = contextManager.getOrCreateContext(`perf-agent-${i}`, 'user-1');
        contextManager.addMessage(context, 'user-1', `Performance test message ${i} about programming`);
        return context;
      });
      ...
      console.log(`Enrichment performance - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per context`);
    ...
    it('should benchmark parallel enrichment performance', async () => {
      ...
      const contexts = Array.from({ length: iterations }, (_, i) => {
        const context = contextManager.getOrCreateContext(`parallel-agent-${i}`, 'user-1');
        contextManager.addMessage(context, 'user-1', `Parallel test message ${i}`);
        return context;
      });
      ...
      console.log(`Parallel enrichment - Sequential: ${sequentialTime.toFixed(2)}ms, Parallel: ${parallelTime.toFixed(2)}ms, Speedup: ${speedup.toFixed(2)}x`);
  ...
  describe('Memory Usage Analysis', () => {
    it('should analyze memory usage during context operations', () => {
      ...
      const contexts = Array.from({ length: iterations }, (_, i) => {
        const context = contextManager.getOrCreateContext(`memory-agent-${i}`, 'user-1');
        contextManager.addMessage(context, 'user-1', `Memory test message ${i} with some longer content to test memory usage patterns`);
        return context;
      });
      ...
      const heapGrowth = (peakMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024; // MB
      const heapRetained = (finalMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024; // MB
      const memoryPerContext = heapRetained / iterations * 1024; // KB
      
      console.log(`Memory analysis - Heap growth: ${heapGrowth.toFixed(2)}MB, Retained: ${heapRetained.toFixed(2)}MB, Per context: ${memoryPerContext.toFixed(2)}KB`);
    ...
    it('should analyze memory usage with enrichment caching', async () => {
      ...
      const contexts = Array.from({ length: iterations }, (_, i) => {
        const context = contextManager.getOrCreateContext(`cache-agent-${i}`, 'user-1');
        contextManager.addMessage(context, 'user-1', `Cache test message ${i} about programming and TypeScript`);
        return context;
      });
      ...
      const enrichmentMemory = (enrichedMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024;
      const cacheOverhead = (cachedMemory.heapUsed - enrichedMemory.heapUsed) / 1024 / 1024;
      
      console.log(`Cache memory analysis - Enrichment: ${enrichmentMemory.toFixed(2)}MB, Cache overhead: ${cacheOverhead.toFixed(2)}MB`);
    ...
    it('should detect memory leaks in long-running operations', async () => {
      ...
      for (let cycle = 0; cycle < cycles; cycle++) {
        ...
        const contexts = Array.from({ length: iterations }, (_, i) => {
          const context = contextManager.getOrCreateContext(`leak-agent-${cycle}-${i}`, 'user-1');
          for (let j = 0; j < 10; j++) {
            contextManager.addMessage(context, 'user-1', `Leak test message ${i}-${j}`);
          }
          return context;
        });
      ...
      console.log('Memory readings (MB):', memoryReadings.map(r => r.toFixed(2)));
    ...
    it('should analyze memory efficiency of different context sizes', () => {
      ...
      const results: Array<{size: number, memory: number, memoryPerMessage: number}> = [];
      
      sizes.forEach(size => {
        const initialMemory = process.memoryUsage().heapUsed;
        
        const context = contextManager.getOrCreateContext(`size-test-${size}`, 'user-1');
        for (let i = 0; i < size; i++) {
          contextManager.addMessage(context, 'user-1', `Size test message ${i} with consistent content length for fair comparison`);
        }
      ...
      results.forEach(({ size, memory, memoryPerMessage }) => {
        console.log(`  ${size} messages: ${memory.toFixed(2)}KB total, ${memoryPerMessage.toFixed(2)}KB per message`);
      });
      
      // Memory per message should not increase significantly with size
      const firstResult = results[0];
      const lastResult = results[results.length - 1];
      const efficiencyRatio = lastResult.memoryPerMessage / firstResult.memoryPerMessage;
  ...
  describe('Context Caching Effectiveness', () => {
    it('should benchmark L1 cache performance', async () => {
      ...
      console.log(`L1 cache performance - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(4)}ms per access`);
    ...
    it('should benchmark L2 cache performance', async () => {
      ...
      console.log(`L2 cache performance - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per access`);
    ...
    it('should benchmark cache coordination performance', async () => {
      ...
      console.log(`Cache coordination performance - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per operation`);
      
      expect(retrieved.every(r => r !== null)).toBe(true);
      expect(avgTime).toBeLessThan(10); // Should coordinate efficiently
      
      const stats = cacheCoordinator.getStats();
      console.log(`Cache stats - Hit rate: ${stats.hitRate.toFixed(2)}, L1 hits: ${stats.l1Hits}, L2 hits: ${stats.l2Hits}`);
    });
    ...
    it('should analyze cache hit rates under different access patterns', async () => {
      ...
      console.log(`Access patterns - Sequential: ${(sequentialEnd - sequentialStart).toFixed(2)}ms, Random: ${(randomEnd - randomStart).toFixed(2)}ms`);
      console.log(`Hit rates - Sequential: ${sequentialStats.hitRate.toFixed(2)}, Random: ${randomStats.hitRate.toFixed(2)}`);
    ...
    it('should benchmark cache eviction performance', async () => {
      ...
      console.log(`Cache eviction performance - Total: ${totalTime.toFixed(2)}ms for ${totalContexts} contexts`);
      
      const stats = cacheCoordinator.getStats();
      console.log(`Eviction stats - Total evictions: ${stats.evictions}, Current size: ${stats.currentSize}`);
  ...
  describe('Multi-Agent Context Sharing Performance', () => {
    it('should benchmark shared context creation', async () => {
      ...
      const sharedContexts = await Promise.all(
        Array.from({ length: iterations }, async (_, i) => {
          const agentSubset = agents.slice(0, Math.min(agentCount, 3 + (i % 3))); // 3-5 agents per context
      ...
      console.log(`Shared context creation - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per context`);
    ...
    it('should benchmark message propagation in shared contexts', async () => {
      ...
      console.log(`Message propagation - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per message`);
    ...
    it('should benchmark context synchronization performance', async () => {
      ...
      console.log(`Context synchronization - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per context`);
    ...
    it('should analyze memory usage in multi-agent scenarios', async () => {
      ...
      const heapGrowth = (peakMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024;
      const heapRetained = (finalMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024;
    ...
    it('should benchmark concurrent access performance', async () => {
      ...
      console.log(`Concurrent access - Total: ${totalTime.toFixed(2)}ms, Average: ${avgTime.toFixed(2)}ms per operation`);
  ...
  describe('Stress Testing and Edge Cases', () => {
    it('should handle extreme load scenarios', async () => {
      ...
      console.log(`Starting extreme load test with ${extremeIterations} contexts...`);
      
      const start = performance.now();
      
      // Process in batches to avoid overwhelming the system
      for (let batch = 0; batch < extremeIterations; batch += batchSize) {
        const batchPromises = Array.from({ length: Math.min(batchSize, extremeIterations - batch) }, (_, i) => {
          const contextId = batch + i;
          const context = contextManager.getOrCreateContext(`extreme-agent-${contextId}`, 'user-1');
          contextManager.addMessage(context, 'user-1', `Extreme load message ${contextId}`);
      ...
      console.log(`Extreme load test - Total: ${(totalTime / 1000).toFixed(2)}s, Average: ${avgTime.toFixed(2)}ms per context`);
    ...
    it('should maintain performance with large message histories', async () => {
      const context = contextManager.getOrCreateContext('large-history-agent', 'user-1');
      ...
      console.log(`Large history performance - Add: ${addTime.toFixed(2)}ms, Operations: ${opTime.toFixed(2)}ms`);
