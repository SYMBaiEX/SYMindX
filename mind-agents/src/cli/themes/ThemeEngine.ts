...
export class ThemeEngine extends EventEmitter {
  ...
  constructor() {
    ...
    Object.keys(cyberpunkTheme.effects).forEach((effect) => {
      if (
        cyberpunkTheme.effects[effect as keyof typeof cyberpunkTheme.effects]
      ) {
  ...
  applyTextStyle(
    ...
    switch (style) {
      ...
      case 'glitch':
        return text
          .split('')
          .map((char) => (Math.random() < 0.1 ? this.getGlitchChar() : char))
          .join('');
      case 'matrix':
        return text
          .split('')
          .map((char) => (Math.random() < 0.2 ? this.getMatrixChar() : char))
          .join('');
      case 'neon':
        return `【 ${text} 】`;
      default:
