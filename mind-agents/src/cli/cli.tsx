...
const showBanner = async () => {
  ...
  console.log(gradient(['#FF006E', '#8338EC', '#3A86FF'])('NEURAL RUNTIME SYSTEM v2.0'))
  console.log()
}
...
program
  ...
  .version('2.0.0')
  .option('-v, --view <view>', 'Initial view (dashboard, agents, chat, logs, settings)', 'dashboard')
  .option('--no-sound', 'Disable sound effects')
  .option('--music', 'Enable background music')
  .option('--theme <theme>', 'Color theme (cyberpunk, matrix, neon, minimal)', 'cyberpunk')
...
const main = async () => {
  ...
  if (!process.stdin.isTTY) {
    ...
    console.error(chalk.white('     cd mind-agents && npx tsx src/cli/cli.tsx\n'))
    console.error(chalk.cyan('  2. Use the CLI commands from the project root:'))
    console.error(chalk.white('     bun cli:dashboard    # Open dashboard view'))
    console.error(chalk.white('     bun cli:agents       # Manage agents'))
    console.error(chalk.white('     bun cli:chat         # Chat with agents\n'))
    console.error(chalk.cyan('  3. Or run the traditional CLI:'))
    console.error(chalk.white('     cd mind-agents && npx tsx src/cli/index.ts\n'))
    console.error(chalk.gray('Note: The Ink-based CLI requires direct terminal access for keyboard input.'))
    process.exit(1)
  }
