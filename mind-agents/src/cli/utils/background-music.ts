...
export class BackgroundMusicManager {
  ...
  async play(trackName: keyof typeof MUSIC_TRACKS, loop: boolean = true): Promise<void> {
    ...
    const musicPath = path.join(__dirname, '..', '..', 'assets', 'music', `${trackName}.mp3`)
    ...
    try {
      if (this.platform === 'darwin') {
        ...
        if (loop) {
          ...
          this.currentProcess = spawn('sh', ['-c', 
        ...
        // Linux - mpg123 or sox
        if (loop) {
          this.currentProcess = spawn('mpg123', ['--loop', '-1', '--scale', 
            Math.floor(this.volume * 32768).toString(), musicPath])
        } else {
          this.currentProcess = spawn('mpg123', ['--scale', 
            Math.floor(this.volume * 32768).toString(), musicPath])
        }
      } else if (this.platform === 'win32') {
