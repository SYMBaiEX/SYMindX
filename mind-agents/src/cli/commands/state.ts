...
export function createStateCommand(): Command {
  const stateCmd = new Command('state')
    .description('Agent state management operations')
  ...
  checkpointCmd
    ...
    .option('-t, --type <type>', 'Checkpoint type (full, incremental, emergency)', 'full')
    .action(async (agentId, options) => {
      try {
        ...
        console.log(`📸 Creating ${checkpointType} checkpoint for agent ${agentId}...`)
        const checkpointPath = await rt.createAgentCheckpoint(agentId, checkpointType)
  ...
  checkpointCmd
    ...
    .action(async (agentId, _options) => {
      try {
        ...
        console.log('Feature not yet implemented - requires state manager exposure')
  ...
  checkpointCmd
    ...
    .action(async (agentId, options) => {
      try {
        ...
        const agent = await rt.restoreAgentFromCheckpoint(agentId, options.checkpoint)
  ...
  stateCmd
    ...
    .action(async (agentId, options) => {
      try {
        if (!options.force) {
          console.log('⚠️  This will force cleanup all resources for the agent.')
  ...
  stateCmd
    ...
    .action(async (options) => {
      try {
        ...
        console.log('\n📊 System Status:')
        console.log(`  State Management: ${systemStatus.enabled ? '✅ Enabled' : '❌ Disabled'}`)
        
        if (systemStatus.enabled) {
          console.log(`  State Directory: ${systemStatus.stateDirectory}`)
          console.log(`  Active Operations: ${systemStatus.activeOperations?.length || 0}`)
          
          if (systemStatus.checkpointSystem) {
            console.log(`  Checkpoint System: ✅ Active`)
            console.log(`    - Total Agents: ${systemStatus.checkpointSystem.totalAgents}`)
            console.log(`    - Active Schedules: ${systemStatus.checkpointSystem.activeSchedules}`)
            console.log(`    - Total Checkpoints: ${systemStatus.checkpointSystem.totalCheckpoints}`)
          }
          
          if (systemStatus.resourceManager) {
            console.log(`  Resource Manager: ✅ Active`)
            console.log(`    - Total Resources: ${systemStatus.resourceManager.totalResources}`)
            console.log(`    - Active Resources: ${systemStatus.resourceManager.activeResources}`)
            console.log(`    - Health Score: ${(systemStatus.resourceManager.health * 100).toFixed(1)}%`)
          }
          
          if (systemStatus.concurrentSafety) {
            console.log(`  Concurrent Safety: ✅ Active`)
            console.log(`    - Active Locks: ${systemStatus.concurrentSafety.totalActiveLocks}`)
            console.log(`    - Queued Requests: ${systemStatus.concurrentSafety.totalQueuedRequests}`)
          }
        }
...
function displaySystemStateStatus(status: any): void {
  ...
  if (status.checkpointSystem) {
    ...
    console.log(`  Active Schedules: ${status.checkpointSystem.activeSchedules}`)
    console.log(`  Total Checkpoints: ${status.checkpointSystem.totalCheckpoints}`)
    console.log(`  Success Rate: ${(status.checkpointSystem.successRate * 100).toFixed(1)}%`)
    console.log(`  Next Checkpoint: ${status.checkpointSystem.nextCheckpoint || 'None scheduled'}`)
  }
  
  if (status.resourceManager) {
    console.log('\n🔧 Resource Manager:')
    console.log(`  Total Resources: ${status.resourceManager.totalResources}`)
    console.log(`  Active Resources: ${status.resourceManager.activeResources}`)
    console.log(`  Stale Resources: ${status.resourceManager.staleResources}`)
    console.log(`  Health Score: ${(status.resourceManager.health * 100).toFixed(1)}%`)
  }
  
  if (status.concurrentSafety) {
    console.log('\n🔒 Concurrent Safety:')
    console.log(`  Active Locks: ${status.concurrentSafety.totalActiveLocks}`)
    console.log(`  Queued Requests: ${status.concurrentSafety.totalQueuedRequests}`)
    console.log(`  Unique Agents: ${status.concurrentSafety.uniqueAgents}`)
    console.log(`  Max Locks/Agent: ${status.concurrentSafety.maxLocksPerAgent}`)
  }
...
function displayAgentStateStatus(agentId: string, status: any): void {
  ...
  if (status.resourceSnapshot) {
    ...
    console.log(`  Total Resources: ${status.resourceSnapshot.summary.totalResources}`)
    console.log(`  Active Resources: ${status.resourceSnapshot.summary.activeResources}`)
    if (status.resourceSnapshot.summary.memoryUsage) {
      console.log(`  Memory Usage: ${(status.resourceSnapshot.summary.memoryUsage / 1024).toFixed(1)} KB`)
    }
  }
  ...
  if (status.checkpointMetrics) {
    console.log('\n📸 Checkpoint Metrics:')
    console.log(`  Total Checkpoints: ${status.checkpointMetrics.totalCheckpoints}`)
    console.log(`  Successful: ${status.checkpointMetrics.successfulCheckpoints}`)
    console.log(`  Failed: ${status.checkpointMetrics.failedCheckpoints}`)
    console.log(`  Average Time: ${status.checkpointMetrics.averageCheckpointTime}ms`)
    if (status.checkpointMetrics.lastCheckpointSize) {
      console.log(`  Last Size: ${(status.checkpointMetrics.lastCheckpointSize / 1024).toFixed(1)} KB`)
    }
  }
