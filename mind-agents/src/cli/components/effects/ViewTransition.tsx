...
interface ViewTransitionProps {
  ...
  variant?: 'fade' | 'slide' | 'zoom' | 'flip' | 'dissolve' | 'glitch' | 'matrix'
...
export const ViewTransition: React.FC<ViewTransitionProps> = ({
  ...
  useEffect(() => {
    if (previousKey.current !== transitionKey && themeEngine.areAnimationsEnabled()) {
  ...
  const renderFlipTransition = () => {
    ...
    const compressionFactor = Math.abs(Math.cos((flipProgress * Math.PI) / 180))
    
    return (
      <Box>
        {!isFlipped ? (
          <Box width={Math.floor(50 * compressionFactor)}>
            {currentContent}
          </Box>
        ) : (
          <Box width={Math.floor(50 * compressionFactor)}>
  ...
  const renderDissolveTransition = () => {
    ...
    const overlay = Array(5).fill(null).map(() => 
      Array(20).fill(null).map(() => {
        const rand = Math.random()
        if (rand < progress) {
          return dissolveChars[Math.min(dissolveIndex, dissolveChars.length - 1)]
        }
        return ' '
      }).join('')
    )
  ...
  const renderGlitchTransition = () => {
    ...
    if (glitchIntensity > 0.3) {
      for (let i = 0; i < 3; i++) {
        const glitchChars = '▓▒░█▄▀■□▢▣▤▥▦▧▨▩▪▫'
        const artifact = Array(10).fill(null).map(() => 
          glitchChars[Math.floor(Math.random() * glitchChars.length)]
        ).join('')
        
        glitchArtifacts.push(
          <Box key={i} marginTop={-1}>
            <Text color={theme.colors.glitch}>
              {artifact}
            </Text>
          </Box>
        )
  ...
  const renderMatrixTransition = () => {
    ...
    if (matrixDensity > 0.2) {
      ...
      for (let i = 0; i < Math.floor(matrixDensity * 5); i++) {
        const line = Array(30).fill(null).map(() => 
