...
export const MatrixRain: React.FC<MatrixRainProps> = ({
  ...
  const charSets = {
    classic: '⌂⌀⌁⌃⌄⌅⌆⌇⌈⌉⌊⌋⌌⌍⌎⌏⌐⌑⌒⌓⌔⌕⌖⌗⌘⌙⌚⌛⌜⌝⌞⌟⌠⌡⌢⌣⌤⌥⌦⌧⌨〈〉⌫⌬アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン',
    binary: '01',
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numDrops; i++) {
      const dropLength = variant === 'binary' ? 8 + Math.floor(Math.random() * 12) : 5 + Math.floor(Math.random() * 10)
      ...
      initialDrops.push({
        ...
        length: dropLength,
        chars: Array.from({ length: dropLength }, () => 
          ...
          if (Math.random() < mutationRate) {
            ...
            if (variant === 'glitch') {
              ...
              for (let i = 0; i < 3; i++) {
                ...
                newChars[idx] = matrixChars[Math.floor(Math.random() * matrixChars.length)] ?? ' '
              }
            } else {
  ...
  const renderMatrix = () => {
    ...
    drops.forEach(drop => {
      for (let i = 0; i < drop.length; i++) {
        ...
        if (y >= 0 && y < height && drop.x >= 0 && drop.x < width && display[y]) {
      ...
      <Text key={y}>
        {row.map((char, x) => {
          ...
          if (dropIndex !== -1) {
            ...
            if (charPosition === 1 && (variant === 'glitch' || variant === 'binary')) {
              return (
