              ...
              <Box>
                {Array.from({ length: explosionSize * 2 + 1 }, (_, i) => (
                  <Text key={i} color={color} dimColor>
                    {explosionChars[Math.min(explosionSize - 1, explosionChars.length - 1)]}
                  </Text>
                ))}
              </Box>
            )}
...
export const SuccessAnimation: React.FC<Omit<StatusAnimationProps, 'type'>> = (props) => (
  <StatusAnimation type="success" {...props} />
)

export const ErrorAnimation: React.FC<Omit<StatusAnimationProps, 'type'>> = (props) => (
  <StatusAnimation type="error" {...props} />
)

export const WarningAnimation: React.FC<Omit<StatusAnimationProps, 'type'>> = (props) => (
  <StatusAnimation type="warning" {...props} />
)

export const InfoAnimation: React.FC<Omit<StatusAnimationProps, 'type'>> = (props) => (
