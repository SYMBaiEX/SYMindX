...
export const AnimatedChart: React.FC<AnimatedChartProps> = ({
  ...
  const width = responsive ? Math.min(dimensions.width - 4, propWidth || 60) : (propWidth || 60)
  ...
  const renderBarChart = () => {
    ...
    data.forEach((point, index) => {
      ...
      const barHeight = Math.floor(normalizedValue * height * animationProgress)
        ...
        for (let x = 0; x < barWidth; x++) {
          if (y >= height - barHeight) {
            const charIndex = Math.min(
              Math.floor((1 - (y - (height - barHeight)) / barHeight) * chars.length),
              chars.length - 1
            )
            row.push(
    ...
    return (
      <Box>
        {data.map((point, index) => {
          const normalizedValue = (point.value - minValue) / range
          const charIndex = Math.floor(normalizedValue * (sparkChars.length - 1) * animationProgress)
