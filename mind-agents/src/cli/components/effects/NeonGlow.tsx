...
export const NeonGlow: React.FC<NeonGlowProps> = ({
  ...
  const getAnimatedColor = (index?: number) => {
    if (!flickerState && animation === 'flicker') return theme.colors.bgSecondary
  ...
  if (variant === 'gradient') {
    return (
      <Box>
        ...
        <Text color={getAnimatedColor()} dimColor={style.dimColor} bold={style.bold}>
          {children}
        </Text>
        <Text color={theme.colors.glowAlt} dimColor>
          {glowChars.right}
    ...
    return (
      <Box>
        ...
        {children.split('').map((char, i) => (
          <Text key={i} color={getAnimatedColor(i)} dimColor={style.dimColor} bold={style.bold}>
            {char}
          </Text>
        ))}
        <Text color={getAnimatedColor()} dimColor={style.dimColor} bold={style.bold}>
