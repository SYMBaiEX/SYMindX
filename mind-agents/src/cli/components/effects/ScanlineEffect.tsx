...
export const ScanlineEffect: React.FC<ScanlineEffectProps> = ({
  ...
  useEffect(() => {
    if (variant === 'static' || variant === 'interference') {
      const noise = Array(height).fill(null).map(() =>
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setScanlinePositions(prev => prev.map((pos, index) => {
        let newPos = pos
        ...
        switch (direction) {
          ...
          case 'up':
            ...
            break
          case 'both':
            newPos = index % 2 === 0 ? (pos + 1) % height : (pos - 1 + height) % height
            break
        }
      ...
      if (variant === 'interference') {
        setStaticNoise(prev => prev.map(row => 
  ...
  const renderScanlines = () => {
    ...
    for (let y = 0; y < height; y++) {
      ...
      const nearScanline = scanlinePositions.some(pos => 
        Math.abs(pos - y) <= 2
      )
      ...
      switch (variant) {
        ...
        case 'digital':
          if (isScanline) {
            lineContent = Array(width).fill(null).map(() => 
              Math.random() < 0.5 ? '1' : '0'
            ).join('')
          } else {
            lineContent = ' '.repeat(width)
        ...
        case 'wave':
          if (isScanline) {
            ...
            lineContent = Array(width).fill(null).map((_, x) => {
              const wave = Math.sin((x / width) * Math.PI * 2 + wavePhase) * 0.5 + 0.5
              if (wave > 0.7) return '▓'
              if (wave > 0.4) return '▒'
              if (wave > 0.2) return '░'
              return ' '
      ...
      if (isScanline) {
        lines.push(
          <Text key={y} color={theme.colors.glow} bold dimColor={variant === 'static'}>
            {lineContent}
          </Text>
        )
