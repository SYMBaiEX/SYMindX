...
interface AnimatedBorderProps {
  width?: number
  height?: number
  variant?: 'solid' | 'dashed' | 'double' | 'rounded' | 'tech' | 'matrix' | 'glow'
...
export const AnimatedBorder: React.FC<AnimatedBorderProps> = ({
  ...
  const getAnimatedColor = (position: 'top' | 'bottom' | 'left' | 'right', index: number) => {
    ...
    switch (animation) {
      case 'pulse':
        return animationFrame % 2 === 0 ? theme.colors.borderBright : defaultColor
        
      case 'flow':
        const flowWindow = 10
        const distance = Math.abs((currentPos - animationFrame + perimeter) % perimeter)
      ...
      case 'snake':
        ...
        const snakeDistance = (currentPos - animationFrame + perimeter) % perimeter
  ...
  const renderMiddle = () => {
    if (!children) {
      return Array.from({ length: height - 2 }, (_, i) => (
        <Box key={i}>
          <Text color={getAnimatedColor('left', height - i - 2)}>{chars.v}</Text>
