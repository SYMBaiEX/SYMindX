...
export const Dashboard: React.FC = () => {
  ...
  const connectionText = connectionStatus ? 'Connected to Runtime' : 'Runtime Offline';
    ...
    <Box flexDirection="column" gap={1}>
      ...
      <Box padding={1} borderStyle="round" borderColor={connectionColor}>
        ...
        {(systemStats.error || agentData.error) && (
          <Text color="red">
            Error: {systemStats.error || agentData.error}
          </Text>
        )}
      </Box>
      ...
      <Box padding={1} borderStyle="round" borderColor={connectionStatus ? "green" : "gray"}>
        <Box flexDirection="column" width="100%">
          <Text bold color={connectionStatus ? "green" : "gray"}>System Overview</Text>
          <Text> </Text>
          <Box flexDirection="row" gap={4}>
            <Box flexDirection="column">
              <Text>
                <Text color="cyan">Runtime Status:</Text> {systemStats.isRunning ? '🟢 Running' : '🔴 Stopped'}
              </Text>
              <Text>
                <Text color="cyan">Uptime:</Text> {systemStats.uptime}
              </Text>
              <Text>
                <Text color="cyan">Memory Usage:</Text> {systemStats.memoryUsage}
              </Text>
            </Box>
            <Box flexDirection="column">
              <Text>
              ...
              <Text>
                <Text color="cyan">Extensions:</Text> {agentData.extensionsCount}
              </Text>
            </Box>
          </Box>
        </Box>
      ...
      <Box padding={1} borderStyle="round" borderColor={connectionStatus ? "yellow" : "gray"}>
        <Box flexDirection="column" width="100%">
          <Text bold color={connectionStatus ? "yellow" : "gray"}>Recent Activity</Text>
          <Text> </Text>
          {agentData.recentActivity.length > 0 ? (
            agentData.recentActivity.slice(0, 5).map((activity, index) => {
              const activityColor = activity.type === 'error' ? 'red' : 
                                    activity.type === 'warning' ? 'yellow' : 'white';
              return (
                <Text key={index} color={activityColor}>
                  <Text color="gray">{activity.timestamp}</Text> - {activity.message}
                </Text>
              );
            })
          ) : (
            <Text color="gray">
              {connectionStatus ? 'No recent activity' : 'Connect to runtime to see activity'}
            </Text>
          )}
        </Box>
      </Box>
      ...
      {systemStats.warnings.length > 0 && (
        <Box padding={1} borderStyle="round" borderColor="red">
          <Box flexDirection="column" width="100%">
            <Text bold color="red">⚠️  System Warnings</Text>
            <Text> </Text>
      ...
      <Box padding={1} borderStyle="round" borderColor="magenta">
        <Box flexDirection="column" width="100%">
          ...
          {!connectionStatus && (
            <>
              <Text> </Text>
              <Text color="gray">💡 Start the SYMindX runtime to enable live data</Text>
            </>
          )}
        </Box>
      </Box>
