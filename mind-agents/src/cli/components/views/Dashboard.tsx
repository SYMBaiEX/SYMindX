...
export const Dashboard: React.FC = (): React.ReactElement => {
  ...
  return (
    <Box flexDirection='column' height='100%'>
        ...
        <Box flexDirection='column' gap={1} width='50%'>
            ...
            <Box flexDirection='column' gap={1}>
              ...
              <Box>
                ...
                <Text color={cyberpunkTheme.colors.warning}>
                  {cyberpunkTheme.ascii.progressFull.repeat(
                    Math.floor((displayStats?.cpu || 0) / 5)
                  )}
                  {cyberpunkTheme.ascii.progressEmpty.repeat(
                    20 - Math.floor((displayStats?.cpu || 0) / 5)
                  )}{' '}
              ...
              <Box>
                ...
                <Text color={cyberpunkTheme.colors.success}>
                  {displayStats
                    ? `${formatBytes(displayStats.memoryUsed)} / ${formatBytes(displayStats.memoryTotal)}`
                    : 'Loading...'}
                </Text>
              </Box>
          ...
          >
            <Chart
              ...
              height={10}
              title={
                selectedMetric === 'cpu' ? 'CPU Usage (%)' : 'Memory Usage (%)'
              }
              color={
                selectedMetric === 'cpu'
            ...
            <Box flexDirection='column' gap={1}>
              {agentData?.agents.slice(0, 5).map((agent) => (
                <Box key={agent.id} flexDirection='column'>
                  ...
                  <Box marginLeft={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>
                      Emotion: {agent.emotion || 'neutral'} | Portal:{' '}
                      {agent.portal || 'none'}
                    </Text>
                  </Box>
      ...
      >
        <Text color={cyberpunkTheme.colors.textDim}>
          [F1] Dashboard | [F2] Agents | [F3] Chat | [F4] Logs | [↑↓] Navigate |
          [Q] Quit
        </Text>
