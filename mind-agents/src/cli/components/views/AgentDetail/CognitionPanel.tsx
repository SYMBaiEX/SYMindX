...
export const CognitionPanel: React.FC<CognitionPanelProps> = ({ agentData }) => {
  ...
  const [viewMode, setViewMode] = useState<'thoughts' | 'goals' | 'decisions'>('thoughts')
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      if (viewMode === 'thoughts' && selectedThought < cognition.currentThoughts.length - 1) {
        setSelectedThought(selectedThought + 1)
      } else if (viewMode === 'goals' && selectedGoal < cognition.activeGoals.length - 1) {
        setSelectedGoal(selectedGoal + 1)
      } else if (viewMode === 'decisions' && selectedDecision < cognition.decisionHistory.length - 1) {
        setSelectedDecision(selectedDecision + 1)
      }
    } else if (input === 'v') {
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        {loopData.map((thought, i) => (
          <Box key={i} flexDirection="column" marginLeft={Math.round(thought.visualDepth * 10)}>
            <Box gap={1}>
  ...
  return (
    <Box flexDirection="column" gap={1}>
        ...
        <Box flexDirection="column" width="30%">
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              <Box gap={2}>
                <Text color={cyberpunkTheme.colors.textDim}>Planning Depth:</Text>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Active Thoughts:</Text>
              ...
              <Box flexDirection="column">
                ...
                <Text color={cyberpunkTheme.colors.success}>
                  {cognition.activeGoals.filter(g => g.status === 'active').length}
                </Text>
              </Box>
              
              <Box flexDirection="column">
                <Text color={cyberpunkTheme.colors.textDim}>Recent Decisions:</Text>
                <Text color={cyberpunkTheme.colors.accent}>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Efficiency Bar:</Text>
                <Text color={cyberpunkTheme.colors.matrix}>
                  {'█'.repeat(Math.round(cognition.planningEfficiency * 15))}
                  {'░'.repeat(15 - Math.round(cognition.planningEfficiency * 15))}
                </Text>
              </Box>
            </Box>
          </Card3D>
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              {viewMode === 'thoughts' && (
                <Box flexDirection="column">
                  <Text color={cyberpunkTheme.colors.textDim}>
                    Current Thoughts ({selectedThought + 1}/{cognition.currentThoughts.length}):
                  </Text>
                  {cognition.currentThoughts.slice(0, 4).map((thought, i) => (
                    <Box
                      ...
                      borderColor={i === selectedThought ? cyberpunkTheme.colors.accent : undefined}
              ...
              {viewMode === 'goals' && (
                <Box flexDirection="column">
                  <Text color={cyberpunkTheme.colors.textDim}>
                    Active Goals ({selectedGoal + 1}/{cognition.activeGoals.length}):
                  </Text>
                  {cognition.activeGoals.slice(0, 3).map((goal, i) => (
                    <Box
                      ...
                      borderColor={i === selectedGoal ? cyberpunkTheme.colors.accent : undefined}
              ...
              {viewMode === 'decisions' && (
                <Box flexDirection="column">
                  <Text color={cyberpunkTheme.colors.textDim}>
                    Recent Decisions ({selectedDecision + 1}/{cognition.decisionHistory.length}):
                  </Text>
                  {cognition.decisionHistory.slice(0, 3).map((decision, i) => (
                    <Box
                      key={i}
                      flexDirection="column"
                      borderStyle={i === selectedDecision ? 'single' : undefined}
                      borderColor={i === selectedDecision ? cyberpunkTheme.colors.accent : undefined}
            ...
            <Box flexDirection="column" width="30%">
              ...
              <Box gap={2}>
                ...
                <Text color={cyberpunkTheme.colors.warning}>
                  {(Math.max(...planningEfficiencyData) - Math.min(...planningEfficiencyData)).toFixed(2)}
                </Text>
              </Box>
            </Box>
          </Box>
        ...
        >
          <Box flexDirection="row" gap={2}>
            ...
            <Box flexDirection="column" width="30%">
              ...
              <Box gap={2}>
                <Text color={cyberpunkTheme.colors.textDim}>Avg Confidence:</Text>
                <Text color={cyberpunkTheme.colors.accent}>
                  {Math.round((decisionConfidenceData.reduce((a, b) => a + b, 0) / decisionConfidenceData.length) * 100)}%
                </Text>
              </Box>
              <Box gap={2}>
      ...
      {viewMode === 'thoughts' && cognition.currentThoughts[selectedThought] && (
        <Box marginTop={1}>
          <Card3D
            title="THOUGHT PROCESS DETAILS"
            width={90}
            height={12}
            color={getThoughtTypeColor(cognition.currentThoughts[selectedThought].type)}
            animated={true}
          >
            <Box flexDirection="column" gap={1}>
              <Box gap={2}>
                ...
                <Text color={cyberpunkTheme.colors.textDim}>Type:</Text>
                <Text color={getThoughtTypeColor(cognition.currentThoughts[selectedThought].type)} bold>
                  {cognition.currentThoughts[selectedThought].type.toUpperCase()}
                </Text>
                <Text color={cyberpunkTheme.colors.textDim}>Confidence:</Text>
                <Text color={cyberpunkTheme.colors.accent}>
                  {Math.round(cognition.currentThoughts[selectedThought].confidence * 100)}%
                </Text>
              </Box>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Related Memories:</Text>
                <Text color={cyberpunkTheme.colors.matrix}>
                  {cognition.currentThoughts[selectedThought].relatedMemories.join(', ')}
                </Text>
              </Box>
              
              <Box gap={2} marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Emotional Context:</Text>
                <Text color={cyberpunkTheme.colors.secondary}>
                  {cognition.currentThoughts[selectedThought].emotionalContext}
                </Text>
                <Text color={cyberpunkTheme.colors.textDim}>Timestamp:</Text>
                <Text color={cyberpunkTheme.colors.text}>
                  {cognition.currentThoughts[selectedThought].timestamp.toLocaleString()}
                </Text>
              </Box>
            </Box>
          </Card3D>
      ...
      {viewMode === 'goals' && cognition.activeGoals[selectedGoal] && (
        <Box marginTop={1}>
          <Card3D
            ...
            width={90}
            height={12}
            color={getGoalStatusColor(cognition.activeGoals[selectedGoal].status)}
            animated={true}
          >
            <Box flexDirection="column" gap={1}>
              <Box gap={2}>
                ...
                <Text color={getGoalStatusColor(cognition.activeGoals[selectedGoal].status)} bold>
                ...
                <Text color={cyberpunkTheme.colors.matrix}>
                  {Math.round(cognition.activeGoals[selectedGoal].progress * 100)}%
                </Text>
              </Box>
              ...
              <Box flexDirection="column" marginTop={1}>
                ...
                <Text color={getGoalStatusColor(cognition.activeGoals[selectedGoal].status)}>
                  {'█'.repeat(Math.round(cognition.activeGoals[selectedGoal].progress * 30))}
                  {'░'.repeat(30 - Math.round(cognition.activeGoals[selectedGoal].progress * 30))}
                </Text>
              </Box>
              ...
              <Box flexDirection="column" marginTop={1}>
                ...
                {cognition.activeGoals[selectedGoal].steps.map((step, i) => (
                  <Box key={i} gap={1}>
                    <Text color={step.completed ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.textDim}>
                      {step.completed ? '✓' : '○'}
                    </Text>
                    <Text color={step.completed ? cyberpunkTheme.colors.text : cyberpunkTheme.colors.textDim}>
                      {step.description}
                    </Text>
                  </Box>
                ))}
      ...
      {viewMode === 'decisions' && cognition.decisionHistory[selectedDecision] && (
        <Box marginTop={1}>
          <Card3D
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              <Box gap={2} marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Timestamp:</Text>
                <Text color={cyberpunkTheme.colors.text}>
                  {cognition.decisionHistory[selectedDecision].timestamp.toLocaleString()}
                </Text>
              </Box>
            </Box>
          </Card3D>
