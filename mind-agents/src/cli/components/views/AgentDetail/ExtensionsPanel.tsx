...
export const ExtensionsPanel: React.FC<ExtensionsPanelProps> = ({ agentData }) => {
  const { extensions } = agentData
  const [selectedExtension, setSelectedExtension] = useState<number>(0)
  const [viewMode, setViewMode] = useState<'overview' | 'usage' | 'errors' | 'performance'>('overview')
  ...
  const getErrorSeverityColor = (severity: 'low' | 'medium' | 'high'): string => {
  ...
  const totalActions = extensions.reduce((sum, ext) => sum + ext.usage.actionsTriggered, 0)
  const totalEvents = extensions.reduce((sum, ext) => sum + ext.usage.eventsHandled, 0)
  const totalErrors = extensions.reduce((sum, ext) => sum + ext.errors.length, 0)
  ...
  return (
    <Box flexDirection="column" gap={1}>
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              <Box gap={2}>
                ...
                <Text color={totalErrors > 0 ? cyberpunkTheme.colors.danger : cyberpunkTheme.colors.success}>
                  {totalErrors}
                </Text>
              </Box>
              
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Health Status:</Text>
                <Text color={runningCount === enabledCount ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.warning}>
                  {'█'.repeat(Math.round((runningCount / Math.max(enabledCount, 1)) * 15))}
                  {'░'.repeat(15 - Math.round((runningCount / Math.max(enabledCount, 1)) * 15))}
                </Text>
            ...
            <Box flexDirection="column" gap={1}>
              ...
              {extensions.map((extension, i) => (
                <Box
                  ...
                  borderColor={i === selectedExtension ? cyberpunkTheme.colors.accent : undefined}
                ...
                >
                  ...
                  <Box marginLeft={2} gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Errors:</Text>
                    <Text color={extension.errors.length > 0 ? cyberpunkTheme.colors.danger : cyberpunkTheme.colors.success}>
                  ...
                  {i === selectedExtension && (
                    <Box marginLeft={2} flexDirection="column" marginTop={1}>
                      <Text color={cyberpunkTheme.colors.textDim}>Last Activity:</Text>
                      <Text color={cyberpunkTheme.colors.text}>
                        {extension.usage.lastActivity.toLocaleString()}
                      </Text>
                      <Text color={cyberpunkTheme.colors.textDim}>Avg Processing:</Text>
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              {viewMode === 'overview' && (
                <Box flexDirection="column" gap={1}>
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Total Actions:</Text>
                    <Text color={cyberpunkTheme.colors.accent}>
                  ...
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Total Events:</Text>
                    <Text color={cyberpunkTheme.colors.secondary}>
                  ...
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Extension Types:</Text>
                    {Array.from(new Set(extensions.map(ext => ext.type))).map((type, i) => {
                      const count = extensions.filter(ext => ext.type === type).length
                      return (
                        <Box key={i} gap={2}>
                  ...
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Status Distribution:</Text>
                    {Array.from(new Set(extensions.map(ext => ext.status))).map((status, i) => {
                      const count = extensions.filter(ext => ext.status === status).length
                      return (
                        <Box key={i} gap={2}>
              ...
              {viewMode === 'usage' && extensions[selectedExtension] && (
                <Box flexDirection="column" gap={1}>
                  ...
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Actions Triggered:</Text>
                    <Text color={cyberpunkTheme.colors.accent}>
                  ...
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Events Handled:</Text>
                    <Text color={cyberpunkTheme.colors.secondary}>
                  ...
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Avg Processing:</Text>
                    <Text color={cyberpunkTheme.colors.warning}>
                      {extensions[selectedExtension].usage.averageProcessingTime}ms
                    </Text>
                  </Box>
                  
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Activity Chart:</Text>
                    <Chart
                  ...
                  <Box gap={2} marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Last Activity:</Text>
                    <Text color={cyberpunkTheme.colors.text}>
                      {extensions[selectedExtension].usage.lastActivity.toLocaleTimeString()}
                    </Text>
                  </Box>
                </Box>
              )}
              ...
              {viewMode === 'errors' && (
                <Box flexDirection="column" gap={1}>
                  ...
                  <Box gap={2}>
                    <Text color={cyberpunkTheme.colors.textDim}>Total Errors:</Text>
                    <Text color={totalErrors > 0 ? cyberpunkTheme.colors.danger : cyberpunkTheme.colors.success}>
                      {totalErrors}
                    </Text>
                  </Box>
                  ...
                  {extensions[selectedExtension] && (
                    <Box flexDirection="column" marginTop={1}>
                      ...
                      {extensions[selectedExtension].errors.length === 0 ? (
                        ...
                        extensions[selectedExtension].errors.slice(0, 3).map((error, i) => (
                          <Box key={i} flexDirection="column" marginTop={1}>
                            <Box gap={1}>
                              <Text color={getErrorSeverityColor(error.severity)}>
                                ●
                              </Text>
                              <Text color={getErrorSeverityColor(error.severity)} bold>
                  ...
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Error Summary by Extension:</Text>
                    {extensions.map((ext, i) => (
                      <Box key={i} gap={2}>
                        <Text color={getExtensionTypeColor(ext.type)}>
                          {ext.name}:
                        </Text>
                        <Text color={ext.errors.length > 0 ? cyberpunkTheme.colors.danger : cyberpunkTheme.colors.success}>
                          {ext.errors.length}
                        </Text>
                      </Box>
                    ))}
              ...
              {viewMode === 'performance' && (
                <Box flexDirection="column" gap={1}>
                  ...
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Processing Times:</Text>
                    {extensions.map((ext, i) => {
                      const maxTime = Math.max(...extensions.map(e => e.usage.averageProcessingTime))
                      const relativeTime = ext.usage.averageProcessingTime / maxTime
                      return (
                        <Box key={i} gap={1}>
                          <Text color={getExtensionTypeColor(ext.type)}>
                            {ext.name.slice(0, 8)}:
                  ...
                  <Box flexDirection="column" marginTop={1}>
                    <Text color={cyberpunkTheme.colors.textDim}>Activity Levels:</Text>
                    {extensions.map((ext, i) => {
                      const maxActivity = Math.max(...extensions.map(e => e.usage.actionsTriggered + e.usage.eventsHandled))
                      const totalActivity = ext.usage.actionsTriggered + ext.usage.eventsHandled
          ...
          >
            <Box flexDirection="row" gap={2}>
              ...
              <Box flexDirection="column" width="20%">
                <Text color={cyberpunkTheme.colors.textDim}>Extension Stats:</Text>
                <Box gap={2}>
                  <Text color={cyberpunkTheme.colors.textDim}>Status:</Text>
                  <Text color={getExtensionStatusColor(extensions[selectedExtension].status)}>
                    {extensions[selectedExtension].status.toUpperCase()}
                  </Text>
                </Box>
                <Box gap={2}>
                  <Text color={cyberpunkTheme.colors.textDim}>Type:</Text>
                  <Text color={getExtensionTypeColor(extensions[selectedExtension].type)}>
                ...
                <Box gap={2}>
                  <Text color={cyberpunkTheme.colors.textDim}>Errors:</Text>
                  <Text color={extensions[selectedExtension].errors.length > 0 ? cyberpunkTheme.colors.danger : cyberpunkTheme.colors.success}>
      ...
      {/* Error Details */}
      {extensions[selectedExtension] && extensions[selectedExtension].errors.length > 0 && (
        <Box marginTop={1}>
          <Card3D
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              {extensions[selectedExtension].errors.length > 3 && (
                <Box marginTop={1}>
                  <Text color={cyberpunkTheme.colors.textDim}>
                    ... and {extensions[selectedExtension].errors.length - 3} more errors
                  </Text>
                </Box>
              )}
            </Box>
