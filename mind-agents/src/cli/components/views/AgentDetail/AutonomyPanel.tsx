...
export const AutonomyPanel: React.FC<AutonomyPanelProps> = ({ agentData }) => {
  ...
  const [viewMode, setViewMode] = useState<'actions' | 'routine' | 'social' | 'curiosity'>('actions')

  useInput((input, key) => {
    if (key.upArrow && selectedAction > 0) {
      setSelectedAction(selectedAction - 1)
    } else if (key.downArrow && selectedAction < autonomy.autonomousActions.length - 1) {
      setSelectedAction(selectedAction + 1)
    } else if (input === 'v') {
  ...
  const actionSuccessRate = autonomy.autonomousActions.length > 0 
    ? autonomy.autonomousActions.filter(a => a.success).length / autonomy.autonomousActions.length 
    : 0

  const averageImpact = autonomy.autonomousActions.length > 0
    ? autonomy.autonomousActions.reduce((sum, a) => sum + a.impact, 0) / autonomy.autonomousActions.length
    : 0

  const routineCompletion = autonomy.dailyRoutine.length > 0
    ? autonomy.dailyRoutine.filter(r => r.completed).length / autonomy.dailyRoutine.length
    : 0

  const socialEffectiveness = autonomy.socialBehaviors.length > 0
    ? autonomy.socialBehaviors.reduce((sum, b) => sum + b.effectiveness, 0) / autonomy.socialBehaviors.length
    : 0
  ...
  const getEthicsColor = (): string => {
    return autonomy.ethicsEnabled ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.danger
  }
      ...
      <Box flexDirection="row" gap={2}>
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Independence Bar:</Text>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Performance Metrics:</Text>
                <Box gap={2}>
            ...
            <Box flexDirection="column" gap={1}>
              ...
              {autonomy.autonomousActions.slice(0, 4).map((action, i) => (
                <Box
                  ...
                  borderColor={i === selectedAction ? cyberpunkTheme.colors.accent : undefined}
                  padding={i === selectedAction ? 1 : 0}
                >
                  <Box gap={1}>
                    <Text color={action.success ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.danger}>
                      {action.success ? '✓' : '✗'}
                    </Text>
                  ...
                  <Text color={cyberpunkTheme.colors.textDim}>
                    Impact: {action.impact.toFixed(2)} | {action.timestamp.toLocaleTimeString()}
                  </Text>
                  {i === selectedAction && (
                    <Box marginTop={1}>
        ...
        <Box flexDirection="column" width="25%">
          ...
          >
            <Box flexDirection="column" gap={1}>
              ...
              <Box flexDirection="column" marginTop={1}>
                ...
                {autonomy.dailyRoutine.map((routine, i) => (
                  <Box key={i} flexDirection="column" marginTop={1}>
                    <Box gap={1}>
                      <Text color={routine.completed ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.textDim}>
                        {routine.completed ? '✓' : '○'}
                      </Text>
              ...
              <Box flexDirection="column" marginTop={1}>
                <Text color={cyberpunkTheme.colors.textDim}>Completion Bar:</Text>
                <Text color={cyberpunkTheme.colors.matrix}>
        ...
        <Box flexDirection="column" width="20%">
          ...
          >
            <Box flexDirection="column" gap={1}>
              <Box gap={2}>
                <Text color={cyberpunkTheme.colors.textDim}>Effectiveness:</Text>
                <Text color={cyberpunkTheme.colors.success}>
          ...
          <Box flexDirection="row" gap={2}>
            ...
            <Box flexDirection="column" width="20%">
              ...
              <Box gap={2}>
                ...
                <Text color={cyberpunkTheme.colors.text}>
                  {Math.round((autonomyScoreData.reduce((a, b) => a + b, 0) / autonomyScoreData.length) * 100)}%
                </Text>
              </Box>
            </Box>
          </Box>
      ...
      <Box marginTop={1}>
        ...
        >
          <Box flexDirection="column" gap={1}>
            ...
            {viewMode === 'actions' && autonomy.autonomousActions[selectedAction] && (
              <Box flexDirection="column" gap={1}>
                <Box gap={2}>
                  <Text color={cyberpunkTheme.colors.textDim}>Action ID:</Text>
                  <Text color={cyberpunkTheme.colors.text}>
                    {autonomy.autonomousActions[selectedAction].id}
                  </Text>
                  <Text color={cyberpunkTheme.colors.textDim}>Success:</Text>
                  <Text color={autonomy.autonomousActions[selectedAction].success ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.danger}>
                    {autonomy.autonomousActions[selectedAction].success ? 'YES' : 'NO'}
                  </Text>
                  <Text color={cyberpunkTheme.colors.textDim}>Impact:</Text>
                  <Text color={cyberpunkTheme.colors.accent}>
                    {autonomy.autonomousActions[selectedAction].impact.toFixed(3)}
                  </Text>
                </Box>
                ...
                <Box gap={2} marginTop={1}>
                  ...
                  <Text color={cyberpunkTheme.colors.text}>
                    {autonomy.autonomousActions[selectedAction].timestamp.toLocaleString()}
                  </Text>
                </Box>
              </Box>
            )}
            ...
            {viewMode === 'curiosity' && (
              <Box flexDirection="column" gap={1}>
                <Box gap={2}>
                  <Text color={cyberpunkTheme.colors.textDim}>Exploration Rate:</Text>
                  <Text color={cyberpunkTheme.colors.matrix}>
                    {Math.round(autonomy.explorationRate * 100)}%
                  </Text>
                </Box>
                ...
                <Box flexDirection="column" marginTop={1}>
                  <Text color={cyberpunkTheme.colors.textDim}>Exploration Progress:</Text>
                  <Text color={cyberpunkTheme.colors.matrix}>
            ...
            {viewMode === 'routine' && (
              <Box flexDirection="column" gap={1}>
                ...
                {autonomy.dailyRoutine.map((routine, i) => (
                  <Box key={i} flexDirection="column" marginTop={1}>
                    <Box gap={2}>
                      <Text color={routine.completed ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.textDim} bold>
                        {routine.time}
                      </Text>
                      <Text color={cyberpunkTheme.colors.textDim}>
                        ({routine.activities.length} activities)
                      </Text>
                      <Text color={routine.completed ? cyberpunkTheme.colors.success : cyberpunkTheme.colors.danger}>
