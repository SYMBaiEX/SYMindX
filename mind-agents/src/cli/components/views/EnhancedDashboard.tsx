...
import { useAgentData, useSystemMetrics, useRuntimeStatus, useRecentEvents } from '../../hooks/useAPIData.js';
...
import { ConnectionStatus, ConnectionHealth, AutoReconnectIndicator } from '../ui/ConnectionStatus.js';
import { ErrorBoundary } from '../ui/ErrorBoundary.js';
import { LoadingIndicator, ProgressBar, Skeleton, Shimmer } from '../ui/LoadingStates.js';

interface EnhancedDashboardProps {
  onSelectAgent?: (agentId: string) => void;
}

export const EnhancedDashboard: React.FC<EnhancedDashboardProps> = ({ onSelectAgent: _onSelectAgent }) => {
  const { dimensions: _dimensions, breakpoints, currentBreakpoint: _currentBreakpoint } = useTerminalDimensions();
  const spacing = getAdaptiveSpacing(breakpoints);
  
  // Connection monitoring
  const { status, stats, isHealthy: _isHealthy, qualityScore, reconnect: _reconnect } = useConnectionMonitor(enhancedRuntimeClient);
  ...
  return (
    <ErrorBoundary onError={(error) => console.error('Dashboard error:', error)} onRetry={handleRetry}>
        ...
        {status.status === 'connected' ? (
          <Box flexDirection="column" gap={1}>
            ...
            <Box flexDirection="column" borderStyle="round" borderColor="gray" padding={1}>
              ...
              {metrics.isLoading ? (
                ...
                <Text color="red">Failed to load metrics: {metrics.error.message}</Text>
              ) : metrics.data ? (
                <Box flexDirection="column">
                  <Text>Uptime: {formatUptime(metrics.data.uptime)}</Text>
                  <Text>Memory: {formatMemory(metrics.data.memory.heapUsed, metrics.data.memory.heapTotal)}</Text>
                  <Box marginTop={1}>
                  ...
                  <Text>Active Agents: {metrics.data.activeAgents}/{metrics.data.totalAgents}</Text>
            ...
            <Box flexDirection="column" borderStyle="round" borderColor="gray" padding={1}>
              ...
              {agents.isLoading ? (
                ...
                <Text color="red">Failed to load agents: {agents.error.message}</Text>
                    ...
                    <Box key={agent.id} marginBottom={1}>
                      ...
                      <Box marginLeft={3}>
                        <Text color="gray" dimColor>
                          Extensions: {agent.extensionCount} | 
                          Portal: {agent.hasPortal ? '✓' : '✗'} | 
                          Ethics: {agent.ethicsEnabled ? 'ON' : 'OFF'}
                        </Text>
                      </Box>
                    </Box>
                  ))}
            ...
            <Box flexDirection="column" borderStyle="round" borderColor="gray" padding={1}>
              ...
              {events.isLoading ? (
                ...
                <Text color="red">Failed to load events: {events.error.message}</Text>
              ) : events.data && events.data.length > 0 ? (
                <Box flexDirection="column">
                  {events.data.slice(0, 5).map((event, i) => (
                    <Box key={i}>
                      <Text color="gray" dimColor>{event.timestamp}</Text>
                      <Text> </Text>
            ...
            {/* Connection Stats */}
            {breakpoints.isMedium || breakpoints.isLarge || breakpoints.isXLarge ? (
              <Box flexDirection="column" borderStyle="round" borderColor="gray" padding={1}>
                ...
                <Text>Min/Max Latency: {stats.minLatency}ms / {stats.maxLatency}ms</Text>
                <Text>Connections: {stats.totalConnections}</Text>
                <Text>Disconnections: {stats.totalDisconnections}</Text>
                {stats.packetLoss > 0 && (
                  <Text color="yellow">Packet Loss: {stats.packetLoss.toFixed(2)}%</Text>
                )}
              </Box>
            ) : null}
          </Box>
          ...
          <Box flexDirection="column" alignItems="center" justifyContent="center" height={10}>
            {status.status === 'connecting' ? (
              <LoadingIndicator variant="spinner" text="Connecting to runtime" color="yellow" />
            ) : (
              <Box flexDirection="column" alignItems="center">
                <Text color="red">⚠️  Not connected to runtime</Text>
                <Box marginTop={1}>
                  <Text color="gray">{status.lastError?.message || 'Unable to establish connection'}</Text>
                </Box>
                <Box marginTop={1}>
                  <Text color="cyan">Press <Text bold>R</Text> to retry</Text>
                </Box>
              </Box>
            )}
          </Box>
