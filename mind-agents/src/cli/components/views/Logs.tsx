...
export const Logs: React.FC = () => {
  ...
  // Generate mock logs
  useEffect(() => {
    const sources = ['runtime', 'agent:nyx', 'agent:aria', 'portal:groq', 'extension:api', 'memory:sqlite']
    const messages = [
  ...
  const getLevelColor = (level: LogEntry['level']) => {
    switch (level) {
      case 'info': return cyberpunkTheme.colors.primary
      case 'warning': return cyberpunkTheme.colors.warning
      case 'error': return cyberpunkTheme.colors.danger
      case 'debug': return cyberpunkTheme.colors.textDim
      case 'success': return cyberpunkTheme.colors.success
    }
  }
      ...
      <Box flexDirection="row" gap={2} flexGrow={1}>
        ...
        <Box flexDirection="column" width="25%">
          ...
          <Box marginTop={1}>
            ...
            >
              <Box flexDirection="column" gap={1}>
                {Array.from(new Set(logs.map(l => l.source))).slice(0, 8).map(source => (
