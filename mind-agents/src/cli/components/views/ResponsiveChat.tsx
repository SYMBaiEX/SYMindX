...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isCompact }) => {
  ...
  const getMessageColor = () => {
    switch (message.type) {
      case 'user': return cyberpunkTheme.colors.accent;
      case 'agent': return cyberpunkTheme.colors.primary;
      case 'system': return cyberpunkTheme.colors.warning;
      default: return cyberpunkTheme.colors.text;
    }
  };
...
export const ResponsiveChat: React.FC = () => {
  ...
  const activeAgents = agentData?.agents.filter(a => a.status === 'active') || [];
  ...
  useInput((_input, key) => {
    if (key.tab) {
      ...
      setSelectedAgentIndex((prev) => 
        (prev + 1) % Math.max(1, activeAgents.length)
      );
    }
  });
  ...
  const chatHeight = getResponsiveValueFromBreakpoints(breakpoints, {
    ...
    >
      ...
      <Header 
        ...
        subtitle={selectedAgent ? 
          (isCompactMode ? selectedAgent.name : `Connected to ${selectedAgent.name}`) : 
          'No agent selected'
        }
        showStatus={true}
              ...
              {activeAgents.length === 0 ? (
                ...
                activeAgents.map((agent, index) => (
                  <ResponsiveBox
                    key={agent.id}
                    padding={1}
                    borderStyle={index === selectedAgentIndex ? 'double' : 'single'}
                    borderColor={index === selectedAgentIndex 
            ...
            <Box flexDirection="column" width="100%">
              ...
              {isCompactMode && activeAgents.length > 0 && (
                <Box marginBottom={1}>
                  ...
                  <Text color={cyberpunkTheme.colors.textDim}> [Tab to switch]</Text>
                </Box>
              )}
              ...
              <Box>
                ...
                <Box flexGrow={1}>
                  <TextInput
                    ...
                    placeholder={isCompactMode ? "Message..." : "Type your message..."}
                  />
                </Box>
              </Box>
            </Box>
