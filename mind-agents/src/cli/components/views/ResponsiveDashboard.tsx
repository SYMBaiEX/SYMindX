...
import { getResponsiveValueFromBreakpoints, shouldShowElement } from '../../utils/responsive-grid.js';
...
export const ResponsiveDashboard: React.FC = () => {
  ...
  const { dimensions, breakpoints, currentBreakpoint: _currentBreakpoint } = useTerminalDimensions();
  ...
  const chartWidth = getResponsiveValueFromBreakpoints(breakpoints, {
  ...
  const chartHeight = getResponsiveValueFromBreakpoints(breakpoints, {
    xs: 6,
    sm: 8,
    md: 10,
    lg: 12,
  ...
  return (
    ...
    >
      ...
      <Header 
        title="SYMINDX" 
        subtitle={shouldShowElement({ xs: false, sm: true, md: true, lg: true, xl: true }, breakpoints.isXSmall ? 'xs' : breakpoints.isSmall ? 'sm' : breakpoints.isMedium ? 'md' : breakpoints.isLarge ? 'lg' : 'xl') ? "NEURAL RUNTIME SYSTEM v2.0" : "v2.0"}
        showStatus={true}
        ...
        >
          ...
          >
            <ResponsiveBox direction="column" gap={1}>
              ...
              <ResponsiveBox show={{ xs: true, sm: true, md: true, lg: true, xl: true }}>
              ...
              <Box>
                ...
                <Text color={cyberpunkTheme.colors.success}>
                  {displayStats ? `${formatBytes(displayStats.memoryUsed)}` : '...'}
                </Text>
              </Box>
              ...
              <ResponsiveBox show={{ xs: false, sm: false, md: true, lg: true, xl: true }}>
          ...
          <ResponsiveBox show={{ xs: false, sm: true, md: true, lg: true, xl: true }}>
            <ResponsiveCard3D 
              title="PERFORMANCE" 
              width={{ xs: 'full', sm: 'full', md: 'auto', lg: 'auto', xl: 'auto' }}
          ...
          >
            <ResponsiveBox direction="column" gap={1}>
              {agentData?.agents.slice(0, getResponsiveValueFromBreakpoints(breakpoints, {
                xs: 3,
                sm: 4,
                md: 5,
                lg: 6,
                ...
                <ResponsiveBox key={agent.id} direction="column">
                  ...
                  <ResponsiveBox marginLeft={2} show={{ xs: false, sm: true, md: true, lg: true, xl: true }}>
                    <Text color={cyberpunkTheme.colors.textDim}>
                      {agent.emotion || 'neutral'} | {agent.portal || 'none'}
                    </Text>
                  </ResponsiveBox>
                  
                  {index < agentData.agents.length - 1 && shouldShowElement({ xs: false, sm: true, md: true, lg: true, xl: true }, breakpoints.isXSmall ? 'xs' : breakpoints.isSmall ? 'sm' : breakpoints.isMedium ? 'md' : breakpoints.isLarge ? 'lg' : 'xl') && (
      ...
      {showMatrix && shouldShowElement({ xs: false, sm: false, md: true, lg: true, xl: true }, breakpoints.isXSmall ? 'xs' : breakpoints.isSmall ? 'sm' : breakpoints.isMedium ? 'md' : breakpoints.isLarge ? 'lg' : 'xl') && (
      ...
      >
        ...
        {musicManager.isEnabled() && shouldShowElement({ xs: false, sm: true, md: true, lg: true, xl: true }, breakpoints.isXSmall ? 'xs' : breakpoints.isSmall ? 'sm' : breakpoints.isMedium ? 'md' : breakpoints.isLarge ? 'lg' : 'xl') && (
          <Text color={cyberpunkTheme.colors.textDim}>
