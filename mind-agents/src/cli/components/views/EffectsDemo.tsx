...
export const EffectsDemo: React.FC = () => {
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      const prevTheme =
        themes[(currentTheme - 1 + themes.length) % themes.length];
      if (prevTheme) {
      ...
      case 'glitch':
        return (
          <Box flexDirection='column' gap={1}>
            ...
            <GlitchText variant='digital' intensity={0.5}>
              Digital Corruption
            </GlitchText>
            <GlitchText variant='matrix' multiLayer>
              Matrix Style Glitch
            </GlitchText>
            <GlitchText variant='chromatic' multiLayer>
      ...
      case 'neon':
        return (
          <Box flexDirection='column' gap={1}>
            ...
            <NeonGlow variant='double' animation='flicker'>
              Flickering Sign
            </NeonGlow>
            <NeonGlow variant='gradient' animation='wave'>
              Wave Animation
            </NeonGlow>
  ...
  return (
    <ViewTransition
      transitionKey={effects[selectedEffect]?.name ?? 'default'}
      variant='slide'
    >
      <Box flexDirection='column' padding={1}>
        <Box marginBottom={1}>
          ...
          <Text dimColor>
            {' '}
            - Use ↑↓ to navigate, ←→ to change theme, 't' to toggle animations
          </Text>
        </Box>
