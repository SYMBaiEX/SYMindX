...
export const Agents: React.FC<AgentsProps> = ({
  ...
  const [agentDetails, setAgentDetails] = useState<AgentDetailInfo | null>(
    null
  );
  ...
  useEffect(() => {
    if (selectedAgent) {
      ...
      setAgentDetails({
        id: selectedAgent,
        name:
          agentData?.agents.find((a) => a.id === selectedAgent)?.name ||
          'Unknown',
        status: 'active',
        emotion: 'confident',
        portal: 'groq',
  ...
  return (
    <Box flexDirection='column' padding={1} gap={1}>
                ...
                <Box flexDirection='column' gap={1}>
                  ...
                  <Box gap={2}>
                    ...
                    <Text color={cyberpunkTheme.colors.text}>
                      {agentDetails.memory.provider} (
                      {agentDetails.memory.entries} entries)
                    </Text>
                  </Box>
      ...
      <Box marginTop={1} gap={3}>
        <Text color={cyberpunkTheme.colors.textDim}>
          [↑↓] Navigate | [Enter/D] Deep Debug | [S] Start/Stop | [R] Restart |
          [E] Edit | [N] New Agent
        </Text>
      </Box>
    </Box>
