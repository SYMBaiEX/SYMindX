    ...
    <Box flexDirection='column' gap={1}>
      ...
      <Box padding={1} borderStyle='round' borderColor={borderColor}>
        <Box flexDirection='row' justifyContent='space-between' width='100%'>
          ...
          <Text color='gray'>
            {connectionStatus
              ? 'Live system data'
              : 'No connection to SYMindX runtime'}
          </Text>
        </Box>
        {systemStats.error && (
          ...
          <Box flexDirection='row' gap={4}>
            <Box flexDirection='column'>
              <Text>
                ...
                <Text color={getStatusColor(systemStats.runtime.status)}>
                  ●
                </Text>{' '}
                {systemStats.runtime.status}
              ...
              <Text>
                ...
                <Text color={getStatusColor(systemStats.memory.status)}>
                  ●
                </Text>{' '}
                {systemStats.memory.status}
              ...
              <Text>
                ...
                <Text color={getStatusColor(systemStats.eventBus.status)}>
                  ●
                </Text>{' '}
                {systemStats.eventBus.status}
            ...
            <Box flexDirection='column'>
              <Text>
                ...
                <Text color={getStatusColor(systemStats.portals.status)}>
                  ●
                </Text>{' '}
                {systemStats.portals.status}
              ...
              <Text>
                ...
                <Text color={getStatusColor(systemStats.extensions.status)}>
                  ●
                </Text>{' '}
                {systemStats.extensions.status}
              ...
              <Text>
                ...
                <Text color={getStatusColor(systemStats.security.status)}>
                  ●
                </Text>{' '}
      ...
      >
        <Box flexDirection='column' width='100%'>
          ...
          <Box flexDirection='row' gap={4}>
            <Box flexDirection='column'>
              <Text>
                ...
                {systemStats.performance.cpu}%
                <Text color='gray'> (heap pressure)</Text>
              </Text>
              <Text>
                <Text color='cyan'>Memory:</Text>{' '}
                {systemStats.performance.memory}
              </Text>
              <Text>
                <Text color='cyan'>Uptime:</Text>{' '}
                {systemStats.performance.uptime}
              </Text>
            </Box>
            <Box flexDirection='column'>
              <Text>
                <Text color='cyan'>Connections:</Text>{' '}
                {systemStats.performance.connections}
              </Text>
              <Text>
                <Text color='cyan'>Commands/min:</Text>{' '}
                {systemStats.performance.messageRate}
              </Text>
              <Text>
                <Text color='cyan'>Response Time:</Text>{' '}
          ...
          {!connectionStatus && (
            <Text color='gray'>
              Performance data requires runtime connection
            </Text>
          )}
        </Box>
      </Box>
      ...
      <Box padding={1} borderStyle='round' borderColor='yellow'>
        <Box flexDirection='column' width='100%'>
          ...
          <Box flexDirection='row' gap={4}>
            <Box flexDirection='column'>
              <Text>
                <Text color='cyan'>Node Version:</Text>{' '}
                {systemStats.environment.nodeVersion}
              </Text>
              <Text>
                <Text color='cyan'>Platform:</Text>{' '}
                {systemStats.environment.platform}
              </Text>
              <Text>
                <Text color='cyan'>Architecture:</Text>{' '}
                {systemStats.environment.arch}
              </Text>
            </Box>
            <Box flexDirection='column'>
              <Text>
                <Text color='cyan'>Config File:</Text>{' '}
                {systemStats.environment.configFile}
              </Text>
              <Text>
                <Text color='cyan'>Log Level:</Text>{' '}
                {systemStats.environment.logLevel}
              </Text>
              <Text>
                <Text color='cyan'>Environment:</Text>{' '}
                {systemStats.environment.env}
              </Text>
            </Box>
          </Box>
        </Box>
        ...
        <Box padding={1} borderStyle='round' borderColor='red'>
          <Box flexDirection='column' width='100%'>
            ...
            {systemStats.warnings.length > 5 && (
              <Text color='gray'>
                ... and {systemStats.warnings.length - 5} more warnings
              </Text>
            )}
          </Box>
        </Box>
      ...
      {!connectionStatus && (
        <Box padding={1} borderStyle='round' borderColor='blue'>
          <Box flexDirection='column' width='100%'>
            ...
            <Text color='gray'>
              • Try: <Text color='cyan'>curl http://localhost:8000/health</Text>
            </Text>
          </Box>
        </Box>
      )}
