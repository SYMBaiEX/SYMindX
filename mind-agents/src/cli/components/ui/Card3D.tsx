...
export const Card3D: React.FC<Card3DProps> = ({
  ...
  const cardDimensions = responsive
    ? getOptimalCardDimensions(dimensions, breakpoints)
    : { width, height };
  ...
  const truncatedTitle = responsive
    ? getResponsiveText(title, actualWidth - 6)
    : title;
  ...
  const topLine =
    ' '.repeat(actualDepth) +
    '╔' +
    '═'.repeat(Math.max(0, actualWidth - 2)) +
    '╗';

  // Depth lines for 3D effect
  const depthLines =
    actualDepth > 0
      ? Array.from({ length: actualDepth - 1 }, (_, i) => {
  ...
  const bottomDepth =
    actualDepth > 0
      ? ' '.repeat(1) + '╲' + '_'.repeat(Math.max(0, actualWidth - 2)) + '╱'
      : '';
