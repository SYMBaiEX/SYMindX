...
export const ResponsiveCard3D: React.FC<ResponsiveCard3DProps> = ({
  ...
  const getWidth = (): number => {
    ...
    if (typeof width === 'object' || width === 'auto' || width === 'full') {
      ...
      if (width === 'auto') {
        const autoWidth = getResponsiveWidth(
          'auto',
          dimensions.width,
          currentBreakpoint
        );
      ...
      const numericWidth =
        getResponsiveValue(width, currentBreakpoint, 40) || 40;
  ...
  const getHeight = (): number => {
    ...
    if (typeof height === 'object' || height === 'auto') {
      if (height === 'auto') {
        const autoHeight = getResponsiveHeight(
          'auto',
          dimensions.height,
          currentBreakpoint
        );
      ...
      const numericHeight =
        getResponsiveValue(height, currentBreakpoint, 10) || 10;
  ...
  const truncatedTitle = responsiveTruncate(
    title,
    titleMaxWidth,
    currentBreakpoint
  );
  ...
  const topLine =
    ' '.repeat(depthValue) + '╔' + '═'.repeat(Math.max(0, cardWidth - 2)) + '╗';

  // Depth lines for 3D effect
  const depthLines =
    depthValue > 0
      ? Array.from({ length: depthValue - 1 }, (_, i) => {
  ...
  const bottomDepth =
    depthValue > 0
      ? ' '.repeat(1) + '╲' + '_'.repeat(Math.max(0, cardWidth - 2)) + '╱'
      : '';
