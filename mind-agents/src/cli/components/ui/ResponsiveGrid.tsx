...
export const GridItem: React.FC<GridItemProps> = ({ 
  ...
  const computedSpan = span ? (getResponsiveValue(span, currentBreakpoint, 1) || 1) : 1;
  
  return (
    <Box
      flexGrow={typeof computedSpan === 'number' ? computedSpan : 1}
      flexBasis={0}
      alignSelf={alignSelf === 'end' ? 'flex-end' : alignSelf === 'start' ? 'flex-start' : alignSelf === 'center' ? 'center' : undefined}
...
export const ResponsiveGrid: React.FC<ResponsiveGridProps> = ({
  ...
  const computedColumns = columns ? getResponsiveColumns(breakpoints, columns) : undefined;
  const computedGap = gap ? getResponsiveValue(gap, currentBreakpoint, 1) || 1 : 1;
  const computedRowGap = rowGap ? getResponsiveValue(rowGap, currentBreakpoint, computedGap) || computedGap : computedGap;
  const computedColumnGap = columnGap ? getResponsiveValue(columnGap, currentBreakpoint, computedGap) || computedGap : computedGap;
  
  // Calculate grid configuration
  const availableWidth = dimensions.width - 4; // Account for padding
  const defaultColumns = computedColumns || (getResponsiveValue({
    xs: 1,
    sm: 2,
    md: 3,
    lg: 4,
    xl: 6,
  }, currentBreakpoint, 3) || 3);
  
  const itemMinWidth = minItemWidth ? (getResponsiveValue(minItemWidth, currentBreakpoint, 20) || 20) : 20;
  const itemMinHeight = minItemHeight ? (getResponsiveValue(minItemHeight, currentBreakpoint, 6) || 6) : 6;
  
  const actualColumns = responsive && (autoFit || autoFill) ?
    Math.min(maxColumns, Math.max(1, Math.floor(availableWidth / (itemMinWidth + (typeof computedGap === 'number' ? computedGap : 1))))) :
    defaultColumns;
  ...
  const gridConfig = {
    ...
    gap: computedGap,
    padding: padding ? (getResponsiveValue(padding, currentBreakpoint, 0) || 0) : 0,
    itemWidth: Math.floor((availableWidth - (actualColumns - 1) * (typeof computedGap === 'number' ? computedGap : 1)) / actualColumns),
  ...
  const itemsPerRow = typeof gridConfig.columns === 'number' ? gridConfig.columns : 1;
  ...
  const itemDimensions = (autoFit || autoFill) && responsive ?
    items.map(() => ({
    ...
    >
      {itemRows.map((row, rowIndex) => (
        <Box
          ...
          justifyContent={
            placeItems === 'start' ? 'flex-start' :
            placeItems === 'center' ? 'center' :
            placeItems === 'end' ? 'flex-end' :
            'space-between'
          }
          alignItems={
            placeItems === 'start' ? 'flex-start' :
        ...
        >
          {row.map((item, itemIndex) => {
            ...
            const itemDims = itemDimensions && itemDimensions[globalIndex] ? itemDimensions[globalIndex] : null;
          ...
          {autoFill && row.length < itemsPerRow && 
            Array.from({ length: Math.max(0, itemsPerRow - row.length) }).map((_, i) => (
              <Box key={`empty-${i}`} flexGrow={1} flexBasis={0} />
            ))
          }
        </Box>
      ))}
