...
interface ResponsiveBoxProps extends Omit<BoxProps, 'flexDirection' | 'justifyContent' | 'alignItems' | 'overflow' | 'overflowX' | 'overflowY' | 'padding' | 'paddingX' | 'paddingY' | 'paddingTop' | 'paddingBottom' | 'paddingLeft' | 'paddingRight' | 'margin' | 'marginX' | 'marginY' | 'marginTop' | 'marginBottom' | 'marginLeft' | 'marginRight' | 'gap' | 'minWidth' | 'maxWidth' | 'minHeight' | 'maxHeight'> {
...
export const ResponsiveBox: React.FC<ResponsiveBoxProps> = ({
  ...
  if (hide) {
    const shouldHide = Object.entries(hide).some(([breakpoint, value]) => {
      ...
      switch (breakpoint) {
        case 'xs': return breakpoints.isXSmall;
        case 'sm': return breakpoints.isSmall;
        case 'md': return breakpoints.isMedium;
        case 'lg': return breakpoints.isLarge;
        case 'xl': return breakpoints.isXLarge;
  ...
  const paddingModel = padding ? getResponsiveBoxModel(padding, undefined, currentBreakpoint) : {
    paddingTop: 0, paddingRight: 0, paddingBottom: 0, paddingLeft: 0,
    marginTop: 0, marginRight: 0, marginBottom: 0, marginLeft: 0
  };
  
  const computedPaddingTop = getResponsiveValue(paddingTop, currentBreakpoint) ?? 
    getResponsiveValue(paddingY, currentBreakpoint) ?? 
    paddingModel.paddingTop;
  const computedPaddingBottom = getResponsiveValue(paddingBottom, currentBreakpoint) ?? 
    getResponsiveValue(paddingY, currentBreakpoint) ?? 
    paddingModel.paddingBottom;
  const computedPaddingLeft = getResponsiveValue(paddingLeft, currentBreakpoint) ?? 
    getResponsiveValue(paddingX, currentBreakpoint) ?? 
    paddingModel.paddingLeft;
  const computedPaddingRight = getResponsiveValue(paddingRight, currentBreakpoint) ?? 
    getResponsiveValue(paddingX, currentBreakpoint) ?? 
    paddingModel.paddingRight;
  
  // Calculate responsive margin
  const marginModel = margin ? getResponsiveBoxModel(undefined, margin, currentBreakpoint) : {
    paddingTop: 0, paddingRight: 0, paddingBottom: 0, paddingLeft: 0,
    marginTop: 0, marginRight: 0, marginBottom: 0, marginLeft: 0
  };
  
  const computedMarginTop = getResponsiveValue(marginTop, currentBreakpoint) ?? 
    getResponsiveValue(marginY, currentBreakpoint) ?? 
    marginModel.marginTop;
  const computedMarginBottom = getResponsiveValue(marginBottom, currentBreakpoint) ?? 
    getResponsiveValue(marginY, currentBreakpoint) ?? 
    marginModel.marginBottom;
  const computedMarginLeft = getResponsiveValue(marginLeft, currentBreakpoint) ?? 
    getResponsiveValue(marginX, currentBreakpoint) ?? 
    marginModel.marginLeft;
  const computedMarginRight = getResponsiveValue(marginRight, currentBreakpoint) ?? 
    getResponsiveValue(marginX, currentBreakpoint) ?? 
    marginModel.marginRight;
  
  // Calculate responsive flex properties
  const flexProps = getResponsiveFlex(direction, justify, align, currentBreakpoint) || {
    flexDirection: 'column',
    justifyContent: 'start',
    alignItems: 'stretch'
  };
  const computedGap = gap ? getResponsiveValue(gap, currentBreakpoint, 0) || 0 : 0;
  ...
  const computedMinWidth = minWidth ? getResponsiveValue(minWidth, currentBreakpoint) : undefined;
  const computedMaxWidth = maxWidth ? getResponsiveValue(maxWidth, currentBreakpoint) : undefined;
  const computedMinHeight = minHeight ? getResponsiveValue(minHeight, currentBreakpoint) : undefined;
  const computedMaxHeight = maxHeight ? getResponsiveValue(maxHeight, currentBreakpoint) : undefined;
  ...
  const content = (
    <Box
      ...
      flexWrap={wrap ? getResponsiveValue(wrap, currentBreakpoint) ? 'wrap' : 'nowrap' : 'nowrap'}
      justifyContent={justifyContentMap[flexProps.justifyContent as keyof typeof justifyContentMap] || 'flex-start'}
      alignItems={alignItemsMap[flexProps.alignItems as keyof typeof alignItemsMap] || 'stretch'}
      gap={computedGap}
