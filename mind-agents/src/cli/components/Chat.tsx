...
export const Chat: React.FC<ChatProps> = ({ agentId }) => {
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      addMessage('System', `Error: ${error instanceof Error ? error.message : 'Unknown error'}`, false);
    } finally {
      setIsLoading(false);
    }
  };

  const addMessage = (from: string, content: string, isUser: boolean) => {
  ...
  return (
    <Box flexDirection="column" height="100%">
      ...
      <Box flexDirection="row" flexGrow={1}>
        ...
        <Box flexDirection="column" width={30} borderStyle="single" borderColor="gray" padding={1}>
          <Text bold color="yellow">Available Agents</Text>
          <Text color="gray">Press 1-5 to select:</Text>
          <Text> </Text>
          {agents?.slice(0, 5).map((agent, index) => (
            <Text key={agent.id} color={selectedAgentId === agent.id ? 'green' : 'white'}>
        ...
        <Box flexDirection="column" flexGrow={1} padding={1}>
          ...
          {/* Input Area */}
          <Box borderStyle="single" borderColor={isInputMode ? 'cyan' : 'gray'} padding={1}>
            <Text bold color={isInputMode ? 'cyan' : 'gray'}>
              {isInputMode ? 'Type your message (Enter to send, ESC to cancel):' : 'Press C to chat'}
            </Text>
            {isInputMode && (
              <Box marginTop={1}>
