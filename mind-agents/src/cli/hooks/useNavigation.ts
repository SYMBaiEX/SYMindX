...
export const useNavigation = (options?: Partial<NavigationHookOptions>) => {
  ...
  const reset = useCallback((item?: NavigationItem) => {
  ...
  const navigate = useCallback((route: string, metadata?: any) => {
    const item: NavigationItem = {
      id: route,
      label: metadata?.title || route.charAt(0).toUpperCase() + route.slice(1),
      data: metadata
    }
    navigateTo(item)
  }, [navigateTo])
...
function buildBreadcrumbs(currentItem: NavigationItem, history: NavigationItem[]): NavigationItem[] {
...
export const getNavigationPath = (item: NavigationItem, allItems: NavigationItem[]): NavigationItem[] => {
