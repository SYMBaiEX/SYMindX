...
export class ContextManager {
  ...
  addMessage(
    ...
    if (context.messages.length > this.config.maxMessageHistory!) {
      context.messages = context.messages.slice(-this.config.maxMessageHistory!)
    }
   ...
   */
  mergeContexts(primaryId: string, secondaryId: string): ConversationContext | null {
  ...
  private analyzeIntent(message: string): string {
    ...
    if (lower.includes('please') || lower.includes('could you')) return 'request'
    ...
    if (lower.includes('i think') || lower.includes('i believe')) return 'opinion'
  ...
  private extractTopics(context: ConversationContext, message: string): void {
    ...
    if (context.topics.length > 0) {
      context.currentTopic = context.topics
        .sort((a, b) => b.mentions - a.mentions)[0].topic
    }
  }
   ...
   */
  private updateMood(context: ConversationContext, message: string, emotion?: string): void {
  ...
  private updatePhase(context: ConversationContext, message: string): void {
    ...
    // Closing detection
             ...
             lower.includes('see you') || lower.includes('talk later')) {
      context.state.phase = 'closing'
    }
    // Active conversation
    else if (context.state.phase === 'greeting' && context.messages.length > 3) {
  ...
  private summarizeContext(context: ConversationContext): string {
    const duration = Math.round((Date.now() - context.startedAt.getTime()) / 60000)
  ...
  private cleanupOldContexts(): void {
    ...
    for (const [id, context] of this.contexts) {
      if (now - context.lastActive.getTime() > this.config.maxContextDuration! * 2) {
        toDelete.push(id)
      }
    }
