...
export class WebSocketServerSkill {
  ...
  getActions(): Record<string, ExtensionAction> {
    return {
      ...
      send_to_connection: {
        ...
        category: ActionCategory.COMMUNICATION,
        parameters: { connectionId: 'string', message: 'string', type: 'string', data: 'object' },
        execute: async (agent: Agent, params: any): Promise<ActionResult> => {
  ...
  async initialize(httpServer?: any): Promise<void> {
    try {
      ...
      runtimeLogger.extension(`🔌 WebSocket server initialized on ${this.config.path}`)
    } catch (error) {
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage): void {
    ...
    if (this.connections.size >= this.config.maxConnections!) {
      runtimeLogger.warn(`⚠️ WebSocket connection limit reached, rejecting connection from ${ip}`)
    ...
    runtimeLogger.extension(`🔌 WebSocket client connected: ${connectionId} from ${ip}`)
    ...
    ws.on('error', (error: Error) => {
      runtimeLogger.error(`❌ WebSocket connection error for ${connectionId}:`, error)
    })
  ...
  private async handleMessage(connection: WebSocketConnection, data: Buffer): Promise<void> {
    try {
      ...
      runtimeLogger.extension(`📨 WebSocket message from ${connection.id}: ${message.type}`)
      ...
      switch (message.type) {
        ...
        case 'command':
          ...
          break
        case 'ping':
          this.sendToWebSocket(connection.ws, { type: 'pong', timestamp: new Date().toISOString() })
          break
        default:
  ...
  private handleSubscription(connection: WebSocketConnection, message: WebSocketMessage): void {
    ...
    if (channel) {
      ...
      runtimeLogger.extension(`📡 Client ${connection.id} subscribed to ${channel}`)
    }
  }
  ...
  private handleUnsubscription(connection: WebSocketConnection, message: WebSocketMessage): void {
    ...
    if (channel) {
      ...
      runtimeLogger.extension(`📡 Client ${connection.id} unsubscribed from ${channel}`)
    }
  }
  ...
  private async handleCommand(connection: WebSocketConnection, message: WebSocketMessage): Promise<void> {
    ...
    try {
      ...
      const result = await this.extension.executeCommand(agentId, data.command, data.parameters)
  ...
  private handleDisconnection(connection: WebSocketConnection, code: number, reason: Buffer): void {
    ...
    runtimeLogger.extension(`🔌 WebSocket client disconnected: ${connection.id} (code: ${code})`)
  }
    ...
    try {
      ...
      for (const connection of this.connections.values()) {
        try {
          ...
          runtimeLogger.warn(`⚠️ Failed to send message to ${connection.id}:`, error)
        }
      }
  ...
  async getConnections(agent: Agent, params: any): Promise<ActionResult> {
    try {
      const connectionInfo: ConnectionInfo[] = Array.from(this.connections.values()).map(conn => ({
        id: conn.id,
  ...
  broadcastAgentEvent(event: AgentEvent): void {
    ...
    for (const connection of this.connections.values()) {
      if (connection.subscriptions.has('agent_events') || connection.subscriptions.has(`agent:${event.agentId}`)) {
        try {
          this.sendToWebSocket(connection.ws, message)
        } catch (error) {
          runtimeLogger.warn(`⚠️ Failed to send agent event to ${connection.id}:`, error)
        }
      }
    }
  }
    ...
    this.heartbeatInterval = setInterval(() => {
      for (const [id, connection] of this.connections.entries()) {
        if (connection.ws.readyState === WebSocket.OPEN) {
          ...
          const timeSinceActivity = Date.now() - connection.clientInfo.lastActivity.getTime()
          if (timeSinceActivity > this.config.heartbeatInterval! * 2) {
            runtimeLogger.warn(`⚠️ Closing inactive WebSocket connection: ${id}`)
