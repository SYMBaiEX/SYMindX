...
export class ExtensionDiscovery {
  ...
  private async discoverNodeModuleExtensions(): Promise<DiscoveredExtension[]> {
    ...
    try {
      ...
      runtimeLogger.warn('⚠️ Failed to discover node_modules extensions:', error)
    }
  ...
  private async discoverLocalExtensions(): Promise<DiscoveredExtension[]> {
    const extensions: DiscoveredExtension[] = []
    const localExtensionsDir = path.join(path.dirname(this.extensionsDir), '..', 'extensions')
        ...
        if (entry.isDirectory()) {
          ...
          if (fs.existsSync(packageJsonPath)) {
            try {
              ...
              runtimeLogger.warn(`⚠️ Failed to parse package.json for local extension ${entry.name}:`, error)
            }
          }
        }
      }
  ...
  async autoRegisterExtensions(registry: ModuleRegistry): Promise<void> {
    ...
    for (const extension of extensions) {
      try {
        ...
        const shouldAutoRegister = extension.packageInfo.symindx?.extension?.autoRegister !== false
   ...
   */
  async registerExtension(registry: ModuleRegistry, extension: DiscoveredExtension): Promise<void> {
    try {
      ...
      if (extension.type === 'built-in') {
        ...
        modulePath = path.join(extension.path, extension.packageInfo.main || 'index')
      }
      ...
      runtimeLogger.error(`❌ Failed to register extension ${extension.name}:`, error)
      throw error
    }
  }
 ...
 */
export function createExtensionDiscovery(projectRoot: string): ExtensionDiscovery {
