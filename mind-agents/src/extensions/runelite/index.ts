...
export class RuneLiteExtension extends EventEmitter implements Extension {
  ...
  public config: RuneLiteConfig & { enabled: boolean };
  ...
  private setupEventHandlers(): void {
    ...
    this.webSocketServer.on('command', (clientId: string, command: RuneLiteCommand) => {
    ...
    this.eventProcessor.on('playerLogin', (data: PlayerLoginData) => {
      runtimeLogger.info('👤 Player login detected');
      this.emit('playerLogin', data);
    });

    this.eventProcessor.on('playerLogout', (data: PlayerLogoutData) => {
      runtimeLogger.info('👤 Player logout detected');
      this.emit('playerLogout', data);
    });

    this.eventProcessor.on('combatStart', (data: CombatStartData) => {
      runtimeLogger.info('⚔️ Combat start detected');
      this.emit('combatStart', data);
    });

    this.eventProcessor.on('combatEnd', (data: CombatEndData) => {
      runtimeLogger.info('⚔️ Combat end detected');
      this.emit('combatEnd', data);
    });

    this.eventProcessor.on('skillExpGained', (data: SkillExpGainedData) => {
      runtimeLogger.debug('📈 Skill exp gained');
      this.emit('skillExpGained', data);
    });

    this.eventProcessor.on('questCompleted', (data: QuestCompletedData) => {
      runtimeLogger.info('🎯 Quest completed');
      this.emit('questCompleted', data);
    });

    this.eventProcessor.on('locationChanged', (data: LocationChangedData) => {
  ...
  private async initializeSkills(agent: Agent): Promise<void> {
    ...
    const skillTrainer = new SkillTrainerSkill({
      ...
      enabled: true,
      gameState: this.eventProcessor.getGameState(),
    });
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      // Cleanup skills
      // await this.skillManager.cleanup(); // Method doesn't exist
  ...
  async getAutonomousStatus(): Promise<Record<string, AutonomousActivityData[]>> {
    const status: Record<string, AutonomousActivityData[]> = {
      skillTraining: [],
      questCompletion: [],
      economicManagement: [],
      socialInteraction: [],
      ...
      case 'skill_training': {
        ...
        if (skillTrainer) {
          if (activityId) {
            ...
            skillTrainer.getAllActivities().forEach((activity: AutonomousActivityData) => {
      ...
      case 'quest_completion': {
        ...
        if (questManager) {
          if (activityId) {
            ...
            questManager.getAllActivities().forEach((activity: AutonomousActivityData) => {
      ...
      case 'economic_management': {
        ...
        if (economicManager) {
          if (activityId) {
            ...
            economicManager.getAllActivities().forEach((activity: AutonomousActivityData) => {
      ...
      case 'social_interaction': {
        ...
        if (socialManager) {
          if (activityId) {
            ...
            socialManager.getAllActivities().forEach((activity: AutonomousActivityData) => {
      ...
      case 'pvp_combat': {
        ...
        if (pvpManager) {
          if (activityId) {
            ...
            pvpManager.getAllActivities().forEach((activity: AutonomousActivityData) => {
  ...
  public updatePlayerInfo(playerInfo: Partial<PlayerLoginData>): void {
  ...
  private async handleCommand(clientId: string, command: RuneLiteCommand): Promise<void> {
