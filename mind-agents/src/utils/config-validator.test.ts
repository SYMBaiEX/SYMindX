...
describe('ConfigValidator', () => {
  ...
  describe('validateEnvironmentConfig', () => {
    ...
    it('should include valid API keys when provided', () => {
      ...
      expect(result.config.apiKeys.GROQ_API_KEY).toBe('gsk_test_key_1234567890123456789012345678901234');
      expect(result.config.apiKeys.OPENAI_API_KEY).toBe('sk-test_key_1234567890123456789012345678901234');
    ...
    it('should reject invalid telegram bot token format', () => {
      ...
      expect(result.warnings).toContain('Invalid format for TELEGRAM_BOT_TOKEN');
    ...
    it('should require at least one AI provider', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('At least one AI provider must be configured with valid API key');
    });
    ...
    it('should validate OpenAI embeddings configuration', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('OpenAI API key required when using OpenAI embeddings');
    });
    ...
    it('should include portal models when configured', () => {
      ...
      expect(result.config.portalModels.GROQ_CHAT_MODEL).toBe('llama-3.1-70b-versatile');
